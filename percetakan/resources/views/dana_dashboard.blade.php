...
{{-- X-DATA harus membungkus SEMUANYA (Tabel & Modal) --}}
<div x-data="{ openModal: null }" x-cloak>
    ...
    {{-- TABEL DATA --}}
    {{-- Container ini memiliki overflow-hidden, makanya Modal jangan ditaruh di sini --}}
                    ...
                    <tr class="hover:bg-blue-50/40 transition-all duration-200 italic">
                        ...
                        <td class="p-6 italic relative">
                            <div class="flex justify-center items-center gap-3 italic">
                                ...
                                {{-- Saat diklik, dia hanya mengubah state openModal. Modalnya sendiri ada di BAWAH --}}
                                <button type="button" 
                                        @click="openModal = {{ $aff->id }}" 
    ...
    {{-- 
        ========================================================================
        MODAL AREA (DITARUH DI LUAR TABEL)
        ========================================================================
        Kita loop ulang affiliatenya khusus untuk render modal.
        Ini cara paling aman agar modal tidak terpotong overflow tabel.
    --}}
    @foreach($affiliates as $aff)
        <template x-if="openModal === {{ $aff->id }}">
            <div class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm" style="text-align: left;">
                
                {{-- Modal Content --}}
                <div class="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden relative border border-slate-100 italic transform transition-all" @click.away="openModal = null">
