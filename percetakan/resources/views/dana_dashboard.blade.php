...
{{-- 1. PASTIKAN ALPINE JS ADA (Hapus baris ini jika di layout utama sudah ada) --}}
<script src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js" defer></script>

<style>
    [x-cloak] { display: none !important; }
    /* Pastikan modal punya z-index tertinggi */
    .modal-z-index { z-index: 9999 !important; }
</style>

{{-- 2. Debugging Text (Akan muncul angka ID saat tombol diklik, buat ngecek fungsi jalan/tidak) --}}
<div x-data="{ openModal: null }" x-cloak class="relative">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-3xl p-6 lg:p-8 mb-8 shadow-lg border border-white/20">
        <div class="flex flex-col md:flex-row justify-between items-center gap-6 text-white">
            ...
            {{-- DEBUGGING: Hapus div ini nanti kalau sudah jalan --}}
            <div class="text-xs bg-black/50 p-2 rounded text-yellow-300">
                Debug ID: <span x-text="openModal"></span>
            </div>
                    ...
                    <tr class="hover:bg-blue-50/40 transition-all duration-200 italic">
                        ...
                        <td class="p-6">
                            <div class="flex items-center space-x-4">
                                <div class="w-12 h-12 bg-indigo-100 rounded-2xl flex items-center justify-center text-indigo-600 font-bold text-xl shadow-inner uppercase">{{ substr($aff->name, 0, 1) }}</div>
                                <div>
                        ...
                        <td class="p-6 text-center italic">
                            <span class="{{ $aff->dana_access_token ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400' }} px-4 py-1.5 rounded-full text-[10px] font-black uppercase border border-slate-200">
                                {{ $aff->dana_access_token ? 'Linked' : 'Offline' }}
                            </span>
                        </td>
                        <td class="p-6 italic">
                            <div class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 italic text-center">Profit Sancaka</div>
                        ...
                        <td class="p-6 italic">
                            <div class="flex justify-center items-center gap-3 italic">
                                ...
                                <form action="{{ route('dana.do_bind') }}" method="POST">
                                    ...
                                    <button class="w-12 h-12 bg-blue-50 text-blue-600 rounded-2xl hover:bg-blue-600 hover:text-white shadow-sm transition-all flex items-center justify-center group">
                                        <i class="fas fa-link text-lg"></i>
                                    </button>
                                </form>

                                {{-- TOMBOL CAIRKAN (TRIGGER) --}}
                                {{-- Menggunakan click.prevent dan stop propagation agar aman --}}
                                <button type="button" 
                                        @click.prevent.stop="openModal = {{ $aff->id }}" 
                                        class="w-12 h-12 bg-rose-50 text-rose-600 rounded-2xl hover:bg-rose-600 hover:text-white shadow-sm transition-all flex items-center justify-center group cursor-pointer">
            ...
            <table class="w-full text-left italic">
                ...
                <tbody class="divide-y divide-slate-50 italic">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors italic">
                        <td class="p-6 text-xs font-semibold text-slate-400 italic">{{ $log->created_at }}</td>
                        <td class="p-6 italic"><span class="px-3 py-1 rounded-xl text-[9px] font-black tracking-widest uppercase italic bg-blue-100 text-blue-600">{{ $log->type }}</span></td>
                        <td class="p-6 text-center font-black text-slate-700 text-sm italic">Rp {{ number_format($log->amount, 0, ',', '.') }}</td>
                        <td class="p-6 font-bold text-emerald-500 text-xs flex items-center gap-2 italic uppercase"><i class="fas fa-circle-check italic"></i> {{ $log->status }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    {{-- 
        ====================================================
        MODAL AREA (DITARUH DI PALING BAWAH, LUAR TABEL)
        ====================================================
        Gunakan x-show dan operator == (bukan ===) agar lebih aman
    --}}
    @foreach($affiliates as $aff)
        <div x-show="openModal == {{ $aff->id }}" 
             style="display: none" {{-- Style bawaan agar tidak flash --}}
             class="fixed inset-0 modal-z-index flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm"
             x-transition.opacity.duration.300ms>
            
            <div class="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden relative border border-slate-100 italic" 
                 @click.away="openModal = null">
                
                <button @click="openModal = null" class="absolute top-6 right-8 text-slate-300 hover:text-slate-600 transition-colors z-10 cursor-pointer">
                    <i class="fas fa-times-circle text-2xl"></i>
                </button>

                <div class="flex flex-col md:flex-row italic text-left">
                    {{-- KIRI: VERIFIKASI --}}
                    <div class="w-full md:w-5/12 bg-slate-50 p-8 border-r border-slate-100 italic">
                        ...
                        <form action="{{ route('dana.account_inquiry') }}" method="POST" class="space-y-6 italic">
                            ...
                            <div class="space-y-2 italic text-center">
                                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest italic text-center">Nomor DANA</label>
                                <input type="text" name="phone" value="{{ $aff->whatsapp }}" class="w-full bg-white border-0 rounded-2xl font-bold py-3 px-4 shadow-sm text-slate-700 italic text-center">
                            </div>
                            @if($aff->dana_user_name)
                            <div class="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl italic text-center">
                                <p class="text-[9px] font-black text-emerald-400 uppercase tracking-widest italic mb-1">Terverifikasi:</p>
                                <p class="text-sm font-black text-emerald-700 italic">{{ $aff->dana_user_name }}</p>
                            </div>
                            @endif
                            <button class="w-full py-4 bg-slate-800 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-900 shadow-lg italic">
                                <i class="fas fa-search mr-2 italic"></i> Cek Akun
                            </button>
                        </form>
                    </div>
                    ...
                    <div class="w-full md:w-7/12 p-8 bg-white italic">
                        <div class="flex items-center justify-between mb-8 italic">
                            ...
                            <div class="text-right italic">
                                <p class="text-[9px] font-black text-slate-300 uppercase italic">Target</p>
                        ...
                        <form action="{{ route('dana.topup') }}" method="POST" class="space-y-6 italic">
                            ...
                            <div class="grid grid-cols-2 gap-4 italic">
                                ...
                                <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 italic text-center">
                                    ...
                                    <p class="text-xs font-bold text-slate-600 italic">Ready</p>
                                </div>
                            </div>

                            <div class="space-y-2 italic text-center">
                                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">Nominal Cair</label>
