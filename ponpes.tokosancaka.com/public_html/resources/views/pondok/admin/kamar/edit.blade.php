...
@section('page_title', 'Edit Data Kamar')

@section('content')
<div class="container mx-auto">
    <div class="bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-xl font-semibold text-gray-700 mb-6">Formulir Edit Kamar</h2>
        ...
        <form action="{{ route('admin.kamar.update', $kamar->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="nama_kamar" name="nama_kamar" value="{{ old('nama_kamar', $kamar->nama_kamar) }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
            </div>
            <div class="mb-4">
                <label for="kapasitas" class="block text-gray-700 text-sm font-medium mb-2">Kapasitas</label>
                <input type="number" id="kapasitas" name="kapasitas" value="{{ old('kapasitas', $kamar->kapasitas) }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="unit_id" name="unit_id" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
                    ...
                    @foreach($unitPendidikan as $unit)
                        <option value="{{ $unit->id }}" {{ old('unit_id', $kamar->unit_id) == $unit->id ? 'selected' : '' }}>{{ $unit->nama_unit }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea id="keterangan" name="keterangan" rows="3" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('keterangan', $kamar->keterangan) }}</textarea>
            </div>
