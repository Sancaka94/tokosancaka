...
@section('content')
                ...
                <form action="{{ route('customer.topup.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <div id="payment-methods-preview" class="mt-4 p-3 bg-gray-50 rounded-lg border border-gray-100 hidden">
                            <div class="flex items-center mb-2">
                                {{-- Logo DANA Header --}}
                                <img src="https://a.m.dana.id/resource/imgs/skywalker/logo/dana-logo-blue.png" class="h-5 mr-2" alt="DANA">
                    ...
                    <div class="space-y-6">
                        <div class="mb-6">
                            ...
                            <select id="payment_method" name="payment_method" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md" required>
    ...
    <script>
        $(document).ready(function() {
            ...
            // --- 1. KAMUS LOGO (Mapping Gambar DANA Tetap) ---
            const logoMap = {
                    ...
                    success: function(response) {
                        ...
                        if(response.success && response.data.length > 0) {
                            ...
                            $.each(response.data, function(index, item) {
                                let apiCode = item.option; // Menggunakan option dari response
                                // Bersihkan nama agar enak dibaca
                                let cleanName = item.method.replace(/_/g, ' ') // Menggunakan method untuk nama
                                                       .replace('VIRTUAL ACCOUNT', 'VA')
                                                       .replace('NETWORK PAY PG', '')
                                                       .replace('DIRECT DEBIT', '');

                                let filename = logoMap[apiCode]; // Mapping berdasarkan option (misal VIRTUAL_ACCOUNT_BCA)
                                let cardContent = '';

                                // UPDATE: Menggunakan rounded-md (kotak) bukan rounded-full
                                if (filename) {
