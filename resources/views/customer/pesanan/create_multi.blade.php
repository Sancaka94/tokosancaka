{{-- 
    File: resources/views/customer/pesanan/create_multi.blade.php
    Deskripsi: Halaman Multi-Koli dengan Sortir Ongkir, New Tab Payment, dan UI Feedback
--}}
...
@php
    // Membuat kunci unik (UUID) untuk mencegah dobel input saat customer submit form
...
<div class="max-w-5xl mx-auto pb-24 pt-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-10 gap-4 px-4">
        ...
        <a href="{{ route('customer.pesanan.create') }}" class="text-sm font-semibold text-blue-700 bg-blue-50 border border-blue-200 px-5 py-2.5 rounded-xl hover:bg-blue-100 transition">
    ...
    <form id="multiOrderForm" action="{{ route('customer.koli.store') }}" method="POST" class="bg-white shadow-xl rounded-2xl border border-gray-100 relative z-0">
        @csrf
        ...
        <div id="step-1" class="step-content active p-6 sm:p-8">
            <div class="mb-6 border-b pb-3 flex items-center">
                ...
                <h2 class="text-lg font-bold text-gray-800">Data Pengirim</h2>
            </div>
            <div class="grid grid-cols-1 gap-6">
                <div class="search-group">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Cari Kontak Tersimpan</label>
                    <div class="relative">
                        <input type="text" id="sender_contact_search" class="w-full border-gray-300 rounded-xl pl-10 py-2.5" placeholder="Ketik nama pengirim..." autocomplete="off">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="sender_name" id="sender_name" class="w-full border-gray-300 rounded-lg text-sm" value="{{ old('sender_name', Auth::user()->nama_lengkap) }}" required>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1 required-label">No. HP</label>
                        <input type="text" name="sender_phone" id="sender_phone" class="w-full border-gray-300 rounded-lg text-sm" value="{{ old('sender_phone', Auth::user()->no_wa) }}" required>
                    </div>
                </div>
                <div class="search-group">
                ...
                <div>
                    ...
                    <textarea name="sender_address" id="sender_address" rows="2" class="w-full border-gray-300 rounded-lg text-sm" required>{{ old('sender_address', Auth::user()->address_detail) }}</textarea>
                </div>
                <div class="pt-2">
        ...
        <div id="step-2" class="step-content p-6 sm:p-8">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div class="search-group">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Cari Kontak Penerima</label>
                    <div class="relative">
                            ...
                            <div class="relative" id="customPaymentDropdown">
                                ...
                                <div id="paymentOptionsListContainer" class="absolute z-50 mt-2 w-full bg-white rounded-xl shadow-xl border border-gray-100 hidden max-h-80 overflow-y-auto">
                                    <ul class="p-2 space-y-1">
                                        {{-- List item akan digenerate/ditampilkan disini --}}
                                        {{-- (Kode list item sama seperti sebelumnya, tapi pastikan di dalam UL ini) --}}
                                        {{-- Saldo --}}
                                        <li class="payment-option cursor-pointer flex items-center p-3 rounded-lg hover:bg-blue-50 transition-colors border-b border-gray-50 last:border-0" 
                                            onclick="selectPayment('Potong Saldo', '{{ asset('public/assets/saldo.png') }}', 'Potong Saldo (Otomatis)')">
                                            ...
                                            <div>
                                                <span class="block text-sm font-bold text-gray-800">Potong Saldo</span>
                                                <span class="block text-xs text-gray-500">
                                                    Saldo {{ Auth::user()->nama_lengkap ?? 'Anda' }}: 
...
<script>
    ...
    const ROUTES = {
        ...
        ongkir: "{{ route('customer.koli.cek_ongkir') }}",
        store_single: "{{ route('customer.koli.store_single') }}"
    };
    ...
    function setupSearch(prefix) {
        ...
        cInput.addEventListener('input', function() {
            ...
            timer = setTimeout(() => {
                fetch(`${ROUTES.contact}?search=${encodeURIComponent(this.value)}&tipe=${prefix==='sender'?'Pengirim':'Penerima'}`)
