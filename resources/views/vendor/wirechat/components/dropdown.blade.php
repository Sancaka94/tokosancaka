...
<div x-ref="button" class="relative" x-data="{ open: false }" @click.outside="open = false" @close.stop="open = false">
    ...
    <div x-show="open"
            ...
            {{$attributes->merge(['class'=>"rounded-lg absolute z-50 mt-2 shadow-lg w-48 bg-[var(--wc-light-secondary)] rounded-md border border-[var(--wc-light-secondary)] shadow-sm overflow-hidden"])}}
            style="display: none;"
            @click="open = false">
        <div>
            {{ $content }}
