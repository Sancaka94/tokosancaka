            ...
            @foreach ($categories as $category)
                <a href="#" class="flex flex-col items-center space-y-2 text-gray-600 hover:text-red-500 transition-colors group">
                    <div class="w-16 h-16 bg-gray-100 rounded-2xl flex items-center justify-center group-hover:bg-red-100 transition-colors">
                        <i class="fas {{ $category['icon'] }} text-3xl text-gray-500 group-hover:text-red-500 transition-colors"></i>
                    </div>
...
<img src="{{ $imageUrl }}"
     ...
     class="w-full h-full object-fill group-hover:scale-105 transition-transform">
                ...
                <div class="flex items-center text-xs text-gray-500 mb-2">
                    ...
                    {{-- Logika baru untuk menampilkan nama toko dari relasi --}}
                    @if ($product->store)
                        <span class="truncate">{{ $product->store->name }}</span>
                    @else
                        {{-- Fallback jika relasi tidak ada (untuk produk lama) --}}
