{{--
    ...
    PERBAIKAN FINAL & STABIL:
    1. Layout HP: Sidebar jadi 'absolute' (melayang) agar konten utama full-width.
    2. Zoom Logic: Dipindah ke INNER WRAPPER agar posisi tombol logout akurat di bawah.
    3. Scrollbar: Muncul di HP, dan di Desktop saat dibuka.
    4. Warna: Hover & Active jadi MERAH.
--}}

<div x-data="{ sidebarOpen: false, isExpanded: false, isHovered: false }" 
     class="h-full flex flex-col absolute lg:static left-0 top-0 z-50">
    ...
    {{-- 
       ASIDE UTAMA (CONTAINER LUAR)
       - Tidak di-zoom agar background biru full h-screen normal.
       - h-screen: memastikan tinggi pas layar.
    --}}
    <aside id="main-sidebar" 
        ...
        class="bg-blue-900 text-gray-300 flex-shrink-0 fixed inset-y-0 left-0 z-50 transform transition-all duration-300 lg:static lg:inset-auto shadow-xl overflow-hidden h-screen">
        
        {{-- 
           INNER ZOOM WRAPPER
           - Zoom 75% disini.
           - Width & Height 133.33% (Kompensasi: 100 / 0.75 = 133.33)
           - flex-col & h-full: Agar logout bisa didorong ke bawah (mt-auto).
        --}}
        <div class="flex flex-col w-[133.33%] h-[133.33%]" style="zoom: 75%;">

            {{-- Tombol Close untuk Mobile --}}
            <div class="flex justify-end p-4 lg:hidden">
                <div id="close-wrapper" class="absolute top-4 right-4 lg:hidden z-50">
            ...
            {{-- SIDEBAR MENU --}}
            {{-- flex-1: Mengisi ruang kosong, mendorong logout ke bawah --}}
            <nav id="sidebar-nav" 
            ...
            {{-- 
               TOMBOL LOGOUT (mt-auto)
               Karena di dalam flex-col & flex-1 nav, mt-auto akan mendorongnya ke bawah
               di dalam container 133% (yang di-zoom jadi 100% visual).
            --}}
