{{--
    ...
    Deskripsi: Topbar/Header untuk panel customer yang sepenuhnya dinamis.
--}}
<header x-data="{ isNotificationsMenuOpen: false, isProfileMenuOpen: false }" class="z-30 py-4 bg-white shadow-md dark:bg-gray-800">
    <div class="container flex items-center justify-between h-full px-6 mx-auto text-indigo-600 dark:text-indigo-300">
        ...
        <!-- Bagian Kiri Kosong (atau bisa diisi search bar nanti) -->
        ...
        <ul class="flex items-center flex-shrink-0 space-x-4 sm:space-x-6">
            ...
            <li class="relative">
                <button class="align-middle rounded-full focus:shadow-outline-indigo focus:outline-none" @click="isProfileMenuOpen = !isProfileMenuOpen" aria-label="Account" aria-haspopup="true">
                    {{-- Path dan Link Logo Profile TIDAK DIUBAH --}}
                ...
                <div x-show="isProfileMenuOpen" @click.away="isProfileMenuOpen = false" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform scale-95" class="absolute right-0 w-56 mt-2 origin-top-right bg-white rounded-md shadow-lg dark:bg-gray-800">
                    <div class="p-2">
                        ...
                        <a class="block w-full px-4 py-2 text-sm text-gray-700 rounded-md hover:bg-indigo-600 hover:text-white dark:hover:bg-gray-700 dark:text-gray-400 dark:hover:text-white" href="{{ route('customer.profile.show') }}">
                            <i class="fas fa-user-circle mr-2"></i> Profil
                        </a>
                        <a class="block w-full px-4 py-2 text-sm text-gray-700 rounded-md hover:bg-indigo-600 hover:text-white dark:hover:bg-gray-700 dark:text-gray-400 dark:hover:text-white" href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
