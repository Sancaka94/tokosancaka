{{--
    ...
    Dropdown notifikasi sekarang menampilkan data dari Activity Log.
--}}
    ...
    <div class="ml-auto flex items-center space-x-2 sm:space-x-4">
        ...
        <div class="hidden md:flex items-center">
            ...
            <span class="ml-2 text-sm font-semibold bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300 py-1 px-3 rounded-full">
             ...
             {{-- PERBAIKAN: Route ini dinonaktifkan sementara untuk mencegah error jika belum ada. Hapus tanda komentar jika route 'wallet.index' sudah siap. --}}
        ...
        <!-- PERBAIKAN: Dropdown Notifikasi sekarang mengambil data dari Activity Log -->
        <div x-data="{ open: false }" class="relative">
            <button @click="open = !open" class="p-2 rounded-full text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none">
                ...
                {{-- Indikator titik merah akan muncul jika ada data notifikasi dari activity log --}}
            ...
            <div x-show="open" @click.away="open = false" x-cloak class="origin-top-right absolute right-0 mt-2 w-80 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none">
                 <div class="py-1">
                    ...
                    <div class="max-h-80 overflow-y-auto custom-scrollbar">
                        {{-- Loop sekarang menggunakan variabel $activityNotifications dari View Composer --}}
                        @forelse($activityNotifications as $log)
                            <div class="block px-4 py-3 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 border-b dark:border-gray-700">
                                ...
                                <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">{{ $log->created_at->diffForHumans() }}</p>
                            </div>
                        @empty
                            <p class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 text-center">Tidak ada aktivitas terbaru.</p>
                        @endforelse
                    </div>
                    {{-- PERBAIKAN: Route ini dinonaktifkan sementara. Hapus komentar jika route 'admin.activity-log.index' sudah siap. --}}
                    <a href="{{-- route('admin.activity-log.index') --}}" class="block text-center px-4 py-2 text-sm text-indigo-600 dark:text-indigo-400 hover:bg-gray-100 dark:hover:bg-gray-700" role="menuitem">Lihat semua</a>
                </div>
            </div>
        </div>
        ...
        <div x-data="{ open: false }" class="relative">
            <button @click="open = !open" class="flex items-center text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                ...
                {{-- PERBAIKAN: Menambahkan gambar profil default jika tidak ada --}}
            ...
            <div x-show="open" @click.away="open = false" x-cloak class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none">
                ...
                {{-- PERBAIKAN: Route ini dinonaktifkan sementara. Hapus komentar jika route 'admin.settings.profile' sudah siap. --}}
