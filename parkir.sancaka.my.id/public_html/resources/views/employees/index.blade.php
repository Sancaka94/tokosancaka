...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Laporan gaji pegawai otomatis dan manual.</p>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-4 text-right text-xs font-bold text-gray-500 uppercase tracking-wider bg-blue-50/50">Bulan Ini</th>
                    <th class="px-6 py-4 text-right text-xs font-bold text-gray-500 uppercase tracking-wider bg-green-50/50">Total Semua Waktu</th>
                    @if(in_array(auth()->user()->role, ['superadmin', 'admin']))
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if($emp->salary_type == 'percentage')
                            <span class="inline-flex px-2 py-0.5 rounded text-xs font-bold bg-blue-100 text-blue-800">Bagi Hasil ({{ (float)$emp->salary_amount }}%)</span>
                        @else
                            <span class="inline-flex px-2 py-0.5 rounded text-xs font-bold bg-gray-100 text-gray-800">Flat (Rp {{ number_format($emp->salary_amount, 0, ',', '.') }})</span>
                        @endif
                    </td>
                ...
                @empty
                <tr><td colspan="5" class="px-6 py-8 text-center text-gray-500 italic">Belum ada data pegawai.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div class="mb-4">
    <h2 class="text-xl font-bold text-gray-800">Riwayat Rincian Gaji Harian</h2>
    <p class="text-sm text-gray-500 mt-1">Lacak gaji otomatis dari sistem vs gaji yang sudah kamu input di Buku Kas.</p>
</div>
...
<div class="bg-white rounded-xl mb-6 border-t-4 border-purple-500 shadow-sm p-4 md:p-5">
    <form action="{{ route('employees.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end">
        <div class="w-full sm:w-auto flex-1 md:flex-none">
            ...
            <input type="date" name="tanggal" value="{{ request('tanggal') }}" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:ring-purple-500">
        </div>

        @if(in_array(auth()->user()->role, ['superadmin', 'admin']))
        <div class="w-full sm:w-auto flex-1 md:flex-none">
            <label class="block text-sm font-bold text-gray-700 mb-1">Cari Nama Pegawai</label>
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Contoh: Dodik" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:ring-purple-500">
        </div>
        @endif

        <div class="flex gap-2 w-full sm:w-auto">
            <button type="submit" class="w-full sm:w-auto bg-gray-800 hover:bg-gray-900 text-white px-6 py-2 rounded-md font-bold shadow-sm">Cari Data</button>
            @if(request()->has('tanggal') || request()->has('search'))
                <a href="{{ route('employees.index') }}" class="w-full sm:w-auto bg-red-50 text-red-600 px-4 py-2 rounded-md font-bold border border-red-200">Reset</a>
            @endif
        </div>
    </form>
</div>
    ...
    <div class="overflow-x-auto p-0">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        @if($history->status == 'manual')
                            <span class="inline-flex items-center gap-1 bg-green-100 text-green-800 px-2.5 py-1 rounded-md text-xs font-bold border border-green-200">
                                ✅ {{ $history->keterangan }}
                            </span>
                        @else
                            <span class="inline-flex items-center gap-1 bg-purple-100 text-purple-800 px-2.5 py-1 rounded-md text-xs font-bold border border-purple-200">
                                🔄 {{ $history->keterangan }}
                            </span>
                        @endif
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-6 py-8 text-center text-gray-500 italic">Data gaji tidak ditemukan untuk filter ini.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
