            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden relative group {{ !$w->is_active ? 'opacity-60 grayscale' : '' }}">
                ...
                <div class="p-5">
                    <div class="mb-4 flex gap-2">
                        ...
                        @if(in_array($w->display_type, ['card', 'employee_salary']))
                        <span class="bg-blue-50 text-blue-700 text-xs font-bold px-3 py-1.5 rounded-lg border border-blue-200 inline-block uppercase">
                    ...
                    <div>
                        <span class="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-2">Rumus Sumber Data</span>
                        <div class="grid grid-cols-2 gap-2 text-sm font-medium">
                            <div class="bg-blue-50 text-blue-800 px-3 py-2 rounded-lg flex justify-between">
                                <span>Parkir</span> <span class="font-black">{{ (float)$w->pct_parkir }}%</span>
                            </div>
                            <div class="bg-indigo-50 text-indigo-800 px-3 py-2 rounded-lg flex justify-between">
                                <span>Nginap</span> <span class="font-black">{{ (float)$w->pct_nginap }}%</span>
                            </div>
                            <div class="bg-emerald-50 text-emerald-800 px-3 py-2 rounded-lg flex justify-between">
                                <span>Toilet</span> <span class="font-black">{{ (float)$w->pct_toilet }}%</span>
                            </div>
                            <div class="bg-amber-50 text-amber-800 px-3 py-2 rounded-lg flex justify-between">
                ...
                <div class="absolute inset-0 bg-slate-900/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                    <button onclick="openModal('edit', {{ $w }})" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg shadow-lg">✏️ Edit</button>
                    <form action="{{ route('admin.builder.destroy', $w->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus widget ini?');">
                        @csrf @method('DELETE')
                        <button type="submit" class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded-lg shadow-lg">🗑️ Hapus</button>
                    </form>
                </div>
            </div>
        @empty
            <div class="col-span-3 bg-white p-10 rounded-2xl shadow-sm text-center">
                Belum ada widget. Buat sekarang!
            </div>
        @endforelse
    </div>
</div>
...
<div id="widgetModal" class="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div class="bg-slate-800 px-6 py-4 flex justify-between items-center sticky top-0 z-10">
            <h2 id="modalTitle" class="text-xl font-bold text-white">⚙️ Buat Widget Baru</h2>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="display_type" id="inp_type" class="w-full border-gray-300 rounded-lg shadow-sm font-bold text-blue-700 bg-blue-50">
                            <option value="card">💳 Kartu Angka (Card)</option>
                            <option value="chart_line">📈 Grafik Garis (Tren 7 Hari)</option>
                            <option value="chart_bar">📊 Grafik Batang (Tren 6 Bulan)</option>
                            <option value="employee_salary">👥 Blok Gaji Pegawai Dinamis</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Judul Widget</label>
                        <input type="text" name="title" id="inp_title" required class="w-full border-gray-300 rounded-lg shadow-sm">
                    </div>
                    ...
                    <div id="wrap_time">
                        <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Waktu (Untuk Kartu & Gaji)</label>
