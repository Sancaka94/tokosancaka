<aside :class="sidebarOpen ? 'w-64 translate-x-0' : 'w-0 -translate-x-full'" class="fixed md:relative inset-y-0 left-0 z-50 flex flex-col h-screen bg-blue-600 text-white transition-all duration-300 ease-in-out flex-shrink-0 shadow-xl overflow-hidden">

    <div class="w-64 h-full flex flex-col">

        <div class="h-16 flex items-center justify-center border-b border-blue-500 px-4 gap-2 shrink-0">
        ...
        <nav class="flex-1 overflow-y-auto py-4">
            <ul class="space-y-1 w-full">
                ...
                {{-- INI KODE BARU YANG DITAMBAHKAN UNTUK RIWAYAT KAS --}}
                <li>
                    <a href="{{ route('kas.index') }}" class="flex items-center gap-2 px-4 py-2 hover:bg-blue-700 transition-colors {{ request()->routeIs('kas.*') ? 'bg-blue-800 border-l-4 border-white' : '' }}">
                        <span>🧾</span> Riwayat Kas Harian
                    </a>
                </li>
                {{-- AKHIR KODE BARU --}}
