{{--
    ...
    Halaman formulir untuk memasukkan OTP dan password baru.
--}}

{{-- PERHATIAN: Tidak menggunakan @extends('layouts.app') agar layout tidak bertabrakan dengan Navbar/Footer utama --}}
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Atur Password Baru - Sancaka Express</title>
    ...
    <style>
        ...
        .password-toggle-icon {
            cursor: pointer;
            background-color: transparent; /* Memastikan background icon transparan */
        }
            ...
            <div class="card card-custom">
                <div class="card-body p-4 p-sm-5">
                    ...
                    <div class="text-center mb-4">
                        {{-- Opsional: Tambahkan Logo jika ada --}}
                        <a href="{{ url('/') }}">
                            <img src="{{ asset('storage/uploads/sancaka.png') }}" alt="Logo Sancaka Express" class="auth-logo" onerror="this.style.display='none'">
                        </a>
                        <i class="bi bi-shield-lock-fill text-primary d-block" style="font-size: 3rem;"></i>
                        <h2 class="h4 fw-bold mt-2">Atur Password Baru</h2>
                        <p class="text-muted small">Masukkan kode OTP dari Email/WA beserta password baru Anda di bawah ini.</p>
                    </div>

                    {{-- Menampilkan Error Validasi Laravel Umum (Misal: Password kurang dari 8 karakter) --}}
                    ...
                    {{-- Form Action diubah menjadi nama route yang baru (menghindari konflik) --}}
                    <form id="otp-form" action="{{ route('password.otp.submit') }}" method="POST">
                        @csrf
                        
                        {{-- Hidden inputs --}}
                        <input type="hidden" name="identifier" value="{{ $identifier ?? request('identifier') }}">
                        <input type="hidden" name="otp" id="otp-hidden" value="">

                        {{-- 1. Info Akun (Readonly) --}}
                        <div class="mb-3">
                            <label class="form-label fw-medium small text-muted">Akun WhatsApp / Email</label>
                            <input type="text" value="{{ $identifier ?? request('identifier') }}" readonly class="form-control form-control-lg bg-light text-muted" style="font-size: 1rem;">
                        </div>

                        {{-- 2. Input 6 Digit OTP --}}
                        <div class="mb-3">
                            <label class="form-label fw-medium small text-muted">Kode OTP (6 Digit)</label>
                        ...
                        <div class="mb-3">
                            <label for="password" class="form-label fw-medium small text-muted">Password Baru</label>
                        ...
                        <div class="mb-4">
                            <label for="password-confirm" class="form-label fw-medium small text-muted">Konfirmasi Password Baru</label>
                            <div class="input-group">
