...
<div class="p-6 lg:p-8 bg-slate-50 min-h-screen">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            {{-- TAMBAHAN: Badge Pickup Point Autokirim --}}
            @if(!empty($user->pickup_point_code))
            <div class="inline-flex items-center px-4 py-2 bg-green-50 border border-green-200 rounded-lg shadow-sm">
                <i class="fas fa-check-circle text-green-500 mr-3 text-2xl"></i>
                <div>
                    <span class="block text-[10px] text-green-600 uppercase font-bold tracking-wider">Pickup Point Aktif</span>
                    <span class="text-sm font-semibold text-green-800">{{ $user->pickup_point_code }}</span>
                </div>
            </div>
            @endif
        </div>
    ...
    function displayResults(results) {
        ...
        results.forEach(item => {
            ...
            div.addEventListener('click', function() {
                ...
                document.getElementById('district_id').value = this.dataset.districtId;
