...
@section('title', $store->name . ' - Sancaka Marketplace')
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mt-8 bg-white rounded-2xl shadow-lg p-6">
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                @php
                    $sellerLogo = $store->user->store_logo_path ?? null;
                    $sellerStoreName = $store->user->store_name ?? $store->name;
                    $sellerCity = $store->user->regency ?? 'Kota Tidak Diketahui';
                    $sellerWa = $store->user->no_wa ?? null;
                    ...
                    $sellerLogoUrl = $sellerLogo
                        ...
                        : 'https://placehold.co/80x80/E2E8F0/4A5568?text=Toko';
                @endphp
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
                    @forelse ($products as $product)
                        <a href="{{ route('products.show', $product->slug) }}"
                           class="bg-white border rounded-xl overflow-hidden group hover:shadow-2xl transition-all duration-300 flex flex-col">

                            <div class="h-48 bg-gray-50 relative">
                                @php
                                    $imageUrl = $product->image_url
                                        ? (Str::startsWith($product->image_url, '/public/storage/')
                                            ? asset($product->image_url)
                                            : asset('public/storage/' . $product->image_url))
                            ...
                            <div class="p-4 flex flex-col flex-grow">
                                ...
                                <div class="mt-auto pt-3">
                                    <form action="{{ route('cart.add', ['product' => $product->id]) }}" method="POST">
                                        ...
                                        <button type="submit"
                                                class="w-full bg-red-500 text-white font-bold py-2.5 rounded-lg text-sm hover:bg-red-600 transition-colors flex items-center justify-center gap-2">
