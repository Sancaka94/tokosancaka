        ...
        <div class="p-4 space-y-4">
            ...
            @auth
                ...
                <form action="{{ Auth::user()->role === 'Admin' ? route('admin.logout') : route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full text-center px-5 py-2.5 font-semibold text-white bg-gray-800 rounded-full mt-2">LOGOUT</button>
                </form>
            @else
                ...
                <a href="{{ route('register') }}" class="block w-full text-center px-5 py-2.5 font-semibold text-white bg-red-500 rounded-full">DAFTAR</a>
            @endauth
        </div>
    </div>
</header>