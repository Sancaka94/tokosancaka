                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="py-3 px-4 text-right">
                                <!-- Aksi Hapus Individual -->
                                <button type="button" onclick="deleteSingle({{ $item->id }})" class="text-gray-400 hover:text-red-600 transition-colors">
...
<div id="bulkEditModal" class="hidden fixed inset-0 z-50 bg-black/60 overflow-y-auto h-full w-full flex items-center justify-center transition-opacity">
    <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md p-6 m-4">
        ...
        <form id="bulkEditForm" action="{{ route('admin.data-autokirim.bulk-update') }}" method="POST">
            @csrf
            <!-- ID yang dipilih akan di-inject ke div ini via JavaScript -->
...
<script>
    ...
    // Mengambil semua ID yang diceklis
    function getSelectedIds() {
        const checkboxes = document.querySelectorAll('.row-checkbox:checked');
        return Array.from(checkboxes).map(cb => cb.value);
    }

    // Fungsi Submit Bulk Destroy
    ...
    function openBulkEditModal() {
        ...
        // Memasukkan array ID ke dalam form modal
    ...
    // Fungsi Hapus Satuan (Individual)
    function deleteSingle(id) {
        if (confirm('Hapus data ini?')) {
            const form = document.getElementById('deleteSingleForm');
            // Pastikan Anda memilik route destroy yang mendukung parameter ID
            form.action = "/admin/data-autokirim/" + id; 
            form.submit();
        }
    }
</script>
