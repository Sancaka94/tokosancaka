                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap align-top">
                            ...
                            <div class="text-xs text-gray-500 mt-1">Nominal: <span class="font-bold text-gray-800 text-sm">Rp {{ number_format($escrow->nominal_ditahan, 0, ',', '.') }}</span></div>
                            ...
                            @if($escrow->status_dana == 'ditahan')
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 mt-2 border border-yellow-200">
                                    <i class="fas fa-hand-paper text-[10px] mr-1.5"></i> Ditahan
                                </span>
                            @elseif($escrow->status_dana == 'dicairkan')
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 mt-2 border border-green-200">
                                    <i class="fas fa-check-double text-[10px] mr-1.5"></i> Telah Cair
                                </span>
                            @elseif($escrow->status_dana == 'mediasi')
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 mt-2 border border-red-200">
                                    <i class="fas fa-gavel text-[10px] mr-1.5"></i> Mediasi
                                </span>
                            @endif
                        ...
                        <td class="px-6 py-4 align-top max-w-sm whitespace-normal">
                            @if($escrow->order)
                                <div class="text-xs text-gray-800 font-medium mb-1">
                                    Resi: <span class="text-blue-600">{{ $escrow->order->shipping_reference ?? 'Belum ada resi' }}</span>
                                </div>
                                <div class="bg-gray-50 p-2 rounded border border-gray-200 mb-2">
                                    <ul class="text-xs text-gray-600 space-y-1 max-h-20 overflow-y-auto pr-1">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center align-top bg-gray-50 border-l border-gray-100">
                            @if($escrow->status_dana === 'ditahan')
                                ...
                                @php
                                    // Cek status order, apakah sudah terkirim/selesai?
                                ...
                                <div class="flex flex-col space-y-2">
                                    @if($isDelivered)
                                        ...
                                        <form action="{{ route('admin.escrow.cairkan', $escrow->id) }}" method="POST" onsubmit="return confirm('PENTING!\nYakin ingin MENGALIRKAN DANA Rp {{ number_format($escrow->nominal_ditahan, 0, ',', '.') }} ke Saldo Penjual?\n\nPastikan pembeli tidak melakukan komplain.');">
                                            ...
                                            <button type="submit" class="w-full inline-flex justify-center items-center px-3 py-2 border border-transparent text-xs font-medium rounded shadow-sm text-white bg-green-600 hover:bg-green-700 transition-colors">
                                                <i class="fas fa-money-bill-wave mr-1.5"></i> Cairkan
                                            </button>
                                        </form>
                                    ...
                                    @else
                                        ...
                                        <button disabled type="button" class="w-full inline-flex justify-center items-center px-3 py-2 border border-gray-200 text-xs font-medium rounded shadow-inner text-gray-400 bg-gray-100 cursor-not-allowed" title="Menunggu barang diterima pembeli">
                                            <i class="fas fa-lock mr-1.5"></i> Cairkan
                                        </button>

                                        <button disabled type="button" class="w-full inline-flex justify-center items-center px-3 py-2 border border-gray-200 text-xs font-medium rounded shadow-inner text-gray-400 bg-gray-50 cursor-not-allowed" title="Menunggu barang diterima pembeli">
                            ...
                            @elseif($escrow->status_dana === 'dicairkan')
                                <div class="text-center p-2">
                                    ...
                                    <p class="text-xs font-bold text-gray-800 mt-0.5">{{ $escrow->dicairkan_pada ? $escrow->dicairkan_pada->format('d M Y') : '-' }}</p>
                                    <p class="text-[10px] text-gray-500">{{ $escrow->dicairkan_pada ? $escrow->dicairkan_pada->format('H:i') : '-' }} WIB</p>
                                </div>
                            @elseif($escrow->status_dana === 'mediasi')
