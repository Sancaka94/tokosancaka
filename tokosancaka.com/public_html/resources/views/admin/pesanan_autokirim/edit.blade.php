...
<div class="max-w-6xl mx-auto px-4 py-8 font-sans" x-data="orderForm">
    <div class="mb-8 border-b border-gray-200 pb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <a href="{{ route('admin.pesanan-autokirim.index') }}"
    ...
    <!-- FORM UTAMA -->
    <form x-ref="orderForm" @input="checkFormValidity()" action="{{ route('admin.pesanan-autokirim.update', $pesanan->id) }}" method="POST" @submit="validateForm($event)" class="space-y-8">
    @csrf
    @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-7 space-y-6">
                   ...
                   <div class="flex items-center justify-between mb-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea id="pengirim_alamat" name="pengirim_alamat" rows="2" required minlength="15" @input.debounce.1000ms="autoGeneratePickup()" placeholder="Contoh: JL SANCAKA EXPRESS NO 15 RT 01 RW 02 DEPAN RSUD" class="uppercase w-full border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-black focus:border-black px-4 py-2.5 bg-white transition duration-200 placeholder-gray-400">{{ old('pengirim_alamat', $pesanan->pengirim_alamat) }}</textarea>
                ...
                <div class="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            <label class="block text-xs font-medium text-gray-700 mb-1.5">NOMOR HP / WA</label>
                            <input type="text" id="penerima_hp" name="penerima_hp" value="{{ old('penerima_hp', $pesanan->penerima_hp) }}" required class="uppercase w-full border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-black focus:border-black px-4 py-2.5 bg-white transition duration-200 placeholder-gray-400">
                        </div>
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea id="penerima_alamat" name="penerima_alamat" rows="2" required minlength="15" placeholder="Contoh: PERUM SANCAKA EXPRESS 2 BLOK A NO 3 RT 04 RW 05" class="uppercase w-full border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-black focus:border-black px-4 py-2.5 bg-white transition duration-200 placeholder-gray-400">{{ old('penerima_alamat', $pesanan->penerima_alamat) }}</textarea>
            ...
            <div class="lg:col-span-5 space-y-6">
                ...
                <div class="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="deskripsi_barang" value="{{ old('deskripsi_barang', $pesanan->deskripsi_barang) }}" placeholder="CONTOH: SEPATU SNEAKERS HITAM UKURAN 42" required class="uppercase w-full border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-black focus:border-black px-4 py-2.5 bg-white placeholder-gray-400">
                        </div>
                ...
                </div> <!-- End Detail Paket -->

                <!-- ======================================================= -->
                <!-- RINGKASAN EKSPEDISI TERPILIH (Bagian Atas) -->
                <!-- ======================================================= -->
                ...
                </div> <!-- End Ringkasan Ekspedisi Terpilih -->
            ...
            <div class="flex flex-col lg:flex-row lg:items-stretch justify-between gap-8">
                ...
                <div class="w-full lg:w-2/3">
                    ...
                    <div x-show="tipePesanan !== 'cod'" x-transition x-cloak>
                        ...
                        <!-- KOTAK INFORMASI DESKRIPSI PEMBAYARAN -->
                        ...
                        <div x-show="selectedPayment && selectedPayment !== 'potong_saldo' && selectedPayment !== 'dana_binding' && selectedPayment !== 'cash'" x-transition class="mt-4 p-4 border border-gray-200 rounded-md text-xs bg-gray-50 flex items-start gap-3" x-cloak>
                    ...
                    <div x-show="tipePesanan === 'cod'" x-transition class="p-4 border border-blue-200 rounded-md bg-blue-50 text-blue-800 text-xs mt-2" x-cloak>
                        <i class="fa-solid fa-info-circle mr-1"></i> Pesanan COD tidak memerlukan metode pembayaran di awal. Tagihan ongkir akan ditagihkan ke pihak penerima oleh kurir.
                    </div>
                </div>
...
<!-- ========================================== -->
@php
    // Pencarian ID Kecamatan berdasarkan Kodepos sebelumnya
    $origin = \App\Models\AutoKirim::where('zip', $pesanan->pengirim_kodepos)->first();
    $dest = \App\Models\AutoKirim::where('zip', $pesanan->penerima_kodepos)->first();

    $pengirim_district_id = $origin ? $origin->district_id : '';
    $penerima_district_id = $dest ? $dest->district_id : '';

    $pengirim_wilayah = $origin ? $origin->district_name . ', ' . $origin->regency_name : 'KODEPOS: ' . $pesanan->pengirim_kodepos;
    $penerima_wilayah = $dest ? $dest->district_name . ', ' . $dest->regency_name : 'KODEPOS: ' . $pesanan->penerima_kodepos;

    // Cek jenis COD
    $metodePay = strtolower($pesanan->metode_pembayaran ?? '');
    $isCodDb = in_array($metodePay, ['cod', 'codbarang', 'cod_barang', 'cod_ongkir']);
    $jenisCodDb = ($metodePay == 'cod_ongkir') ? 'cod_ongkir' : 'cod_barang';
@endphp

<script>
document.addEventListener('alpine:init', () => {
    Alpine.data('orderForm', () => ({
        // INJEKSI DATA LAMA ($pesanan) KE VARIABEL
        kategoriBarang: '{!! old('kategori_barang', $pesanan->kategori_barang) !!}',
        tipePesanan: '{{ old('tipe_pesanan', $isCodDb ? 'cod' : 'reguler') }}',
        jenisCod: '{{ old('jenis_cod_radio', $jenisCodDb) }}',
        simpanPengirim: false,
        simpanPenerima: false,
        resiCashless: '',

        nilaiBarang: '{{ old('nilai_barang', $pesanan->nilai_barang) }}',
        berat: '{{ old('berat_gram', $pesanan->berat_gram) }}',
        isSubmitting: false,
        qty: 1,
        isSenderPp: 1,
        asuransi: {{ old('asuransi', $pesanan->asuransi) == 1 ? 'true' : 'false' }},
        panjang: '{{ old('panjang_cm', $pesanan->panjang_cm) }}',
        lebar: '{{ old('lebar_cm', $pesanan->lebar_cm) }}',
        tinggi: '{{ old('tinggi_cm', $pesanan->tinggi_cm) }}',

        senderQuery: '{!! old('senderQuery', $pengirim_wilayah) !!}',
        senderDistrictId: '{{ old('pengirim_district_id', $pengirim_district_id) }}',
        senderPostalCode: '{{ old('pengirim_kodepos', $pesanan->pengirim_kodepos) }}',
        ...
        receiverQuery: '{!! old('receiverQuery', $penerima_wilayah) !!}',
        receiverDistrictId: '{{ old('penerima_district_id', $penerima_district_id) }}',
        receiverPostalCode: '{{ old('penerima_kodepos', $pesanan->penerima_kodepos) }}',
        ...
        selectedPayment: '{!! old('metode_pembayaran', $pesanan->metode_pembayaran) !!}',
        showPaymentModal: false,
        selectedPaymentName: '{!! old('metode_pembayaran', str_replace('_', ' ', strtoupper($pesanan->metode_pembayaran))) !!}',
        selectedPaymentIcon: '',

        // Variabel Kontak Pengirim
        pengirimNama: '{!! old('pengirim_nama', $pesanan->pengirim_nama) !!}',
        contactResultsSender: [],
        showContactSender: false,
        isSearchingContactSender: false,
        pickupPointCode: '{{ auth()->user()->pickup_point_code ?? '' }}',

        displayBerat: '{{ old('berat_gram', $pesanan->berat_gram) ? number_format(old('berat_gram', $pesanan->berat_gram), 0, ',', '.') : '' }}',
        displayNilaiBarang: '{{ old('nilai_barang', $pesanan->nilai_barang) ? "Rp " . number_format(old('nilai_barang', $pesanan->nilai_barang), 0, ',', '.') : '' }}',

        // Variabel Kontak Penerima
        penerimaNama: '{!! old('penerima_nama', $pesanan->penerima_nama) !!}',
        ...
        async autoGeneratePickup() {
            ...
            if (nama.length >= 2 && hp.length >= 9 && districtId && alamat.length >= 15) {
                ...
                try {
                    ...
                    let response = await fetch("{{ route('admin.pesanan-autokirim.ajax-pickup') }}", {
                        method: 'POST',
                        body: formData
                    });
