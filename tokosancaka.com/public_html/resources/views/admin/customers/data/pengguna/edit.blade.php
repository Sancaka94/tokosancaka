...
<div class="py-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Edit Data Akun Pengguna</h1>
        <a href="{{ route('admin.customers.data.pengguna.index') }}" class="mt-4 sm:mt-0 inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
    ...
    <form action="{{ route('admin.customers.data.pengguna.update', $data->id_pengguna) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow overflow-hidden sm:rounded-lg">
        @csrf
        @method('PUT')
        ...
        <div class="p-4 sm:p-6 space-y-8 sm:space-y-10">
            ...
            <div>
                <div class="border-b border-gray-200 pb-4 mb-5">
                    <h3 class="text-lg font-bold leading-6 text-indigo-900 flex items-center">
                ...
                <div class="grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-2 lg:grid-cols-2">
                    ...
                    <div class="col-span-1">
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email', $data->email) }}" required
                                   class="block w-full pl-10 sm:text-sm border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 transition duration-150 ease-in-out">
                        </div>
                    ...
                    {{-- Role --}}
                    <div class="col-span-1">
                        <label for="role" class="block text-sm font-semibold text-gray-700">Peran (Role) <span class="text-red-500">*</span></label>
                        <select id="role" name="role" required class="mt-1 block w-full bg-white rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm transition duration-150 ease-in-out">
                    ...
                    {{-- Status Akun --}}
                    <div class="col-span-1">
                        <label for="status" class="block text-sm font-semibold text-gray-700">Status Akun <span class="text-red-500">*</span></label>
                        <select id="status" name="status" required class="mt-1 block w-full bg-white rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm transition duration-150 ease-in-out">
            ...
            <div>
                <div class="border-b border-gray-200 pb-4 mb-5">
                    <h3 class="text-lg font-bold leading-6 text-teal-900 flex items-center">
                ...
                <div class="grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-2 lg:grid-cols-2">
                    ...
                    {{-- Nama Toko --}}
                    <div class="col-span-1">
                        <label for="store_name" class="block text-sm font-semibold text-gray-700">Nama Toko</label>
                        <input type="text" name="store_name" id="store_name" value="{{ old('store_name', $data->store_name) }}" placeholder="Contoh: Toko Sancaka"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500 sm:text-sm transition duration-150 ease-in-out">
                    ...
                    <div class="col-span-1">
                        ...
                        <div class="flex items-center space-x-4 p-3 bg-gray-50 rounded-md border border-gray-200">
                            @if ($data->store_logo_path)
                                <img src="{{ asset('public/storage/' . $data->store_logo_path) }}" alt="Logo Toko" class="h-12 w-12 object-cover rounded-md shadow-sm border border-gray-300">
                            @else
                                <div class="h-12 w-12 rounded-md bg-gray-200 flex items-center justify-center border border-gray-300">
                    ...
                    <div class="sm:col-span-2">
                        <label for="address_detail" class="block text-sm font-semibold text-gray-700">Detail Alamat (Jalan, No. Rumah, RT/RW)</label>
                        <textarea id="address_detail" name="address_detail" rows="3" placeholder="Masukkan alamat lengkap pengiriman/toko..."
                                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500 sm:text-sm transition duration-150 ease-in-out">{{ old('address_detail', $data->address_detail) }}</textarea>
                        @error('address_detail')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
                    </div>

                    {{-- Info Geografis (Readonly / Visual Info) --}}
                    <div class="sm:col-span-2">
                        <div class="bg-blue-50 p-4 rounded-md border border-blue-100 flex flex-col sm:flex-row sm:items-center sm:space-x-8">
                            <div class="mb-3 sm:mb-0">
                                <span class="block text-xs font-semibold text-blue-800 uppercase tracking-wider">Provinsi Terdaftar</span>
                                <span class="block mt-1 text-sm text-blue-900 font-medium">{{ $data->province ?? 'Belum Diatur' }}</span>
                            </div>
                            <div>
                                <span class="block text-xs font-semibold text-blue-800 uppercase tracking-wider">Kota/Kabupaten</span>
                                <span class="block mt-1 text-sm text-blue-900 font-medium">{{ $data->regency ?? 'Belum Diatur' }}</span>
                            </div>
                        </div>
                    </div>
            ...
            <div>
                <div class="border-b border-gray-200 pb-4 mb-5">
                    <h3 class="text-lg font-bold leading-6 text-green-800 flex items-center">
                        <svg class="w-5 h-5 mr-2 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg>
                ...
                <div class="grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-2 lg:grid-cols-2">
                    ...
                    {{-- Nama Bank --}}
                    <div class="col-span-1">
                        <label for="bank_name" class="block text-sm font-semibold text-gray-700">Nama Bank</label>
                        <input type="text" name="bank_name" id="bank_name" value="{{ old('bank_name', $data->bank_name) }}" placeholder="Contoh: BCA, Mandiri, BRI"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition duration-150 ease-in-out">
                    </div>

                    {{-- Nomor Rekening --}}
                    <div class="col-span-1">
                        <label for="bank_account_number" class="block text-sm font-semibold text-gray-700">Nomor Rekening</label>
                        <input type="text" name="bank_account_number" id="bank_account_number" value="{{ old('bank_account_number', $data->bank_account_number) }}" placeholder="Contoh: 1234567890"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition duration-150 ease-in-out font-mono">
                    </div>

                    {{-- Nama Pemilik Rekening --}}
                    <div class="sm:col-span-2">
                        <label for="bank_account_name" class="block text-sm font-semibold text-gray-700">Nama Pemilik Rekening (Sesuai Buku Tabungan)</label>
                        <input type="text" name="bank_account_name" id="bank_account_name" value="{{ old('bank_account_name', $data->bank_account_name) }}" placeholder="Contoh: JOHN DOE"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm transition duration-150 ease-in-out uppercase">
                    </div>
                </div>
            </div>

            {{-- BLOK 4: KEAMANAN (PASSWORD & PIN) --}}
            <div class="bg-red-50 -mx-4 sm:-mx-6 -mb-6 p-4 sm:p-6 sm:pb-8 border-t border-red-100 rounded-b-lg">
                <div class="border-b border-red-200 pb-4 mb-5">
                    <h3 class="text-lg font-bold leading-6 text-red-800 flex items-center">
                        <svg class="w-5 h-5 mr-2 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                        Keamanan Akun (Password & PIN)
                    </h3>
                    <p class="mt-1 text-sm text-red-600 font-medium">PERHATIAN: Kosongkan kolom di bawah ini jika Anda TIDAK INGIN mengubah kredensial pengguna.</p>
                </div>

                <div class="grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-2 lg:grid-cols-2">

                    {{-- 4A: BAGIAN PASSWORD LOGIN --}}
                    <div class="sm:col-span-2 bg-white p-4 rounded-md shadow-sm border border-red-100">
                        <h4 class="text-md font-semibold text-gray-800 mb-4 border-b pb-2">🔑 Password Login Aplikasi</h4>

                        <div class="grid grid-cols-1 gap-y-4 sm:grid-cols-2 sm:gap-x-4">
                            <div class="sm:col-span-2">
                                <label for="current_hash" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Hash Password Saat Ini</label>
                                <div class="relative">
                                    <input type="password" id="current_hash" value="{{ $data->password_hash ?? 'Tidak Ada Hash' }}" readonly
                                           class="block w-full rounded-md border-gray-200 bg-gray-100 text-gray-500 sm:text-sm pr-10 font-mono text-xs cursor-not-allowed">
                            ...
                            <div class="col-span-1">
                                <label for="password_confirmation" class="block text-sm font-medium text-gray-700">Ketik Ulang Password Baru</label>
                    ...
                    {{-- 4B: BAGIAN PIN TRANSAKSI (TAMBAHAN BARU) --}}
                    <div class="sm:col-span-2 bg-white p-4 rounded-md shadow-sm border border-red-100">
                        <h4 class="text-md font-semibold text-gray-800 mb-4 border-b pb-2 flex justify-between items-center">
                            <span>🛡️ PIN Transaksi (Keuangan)</span>
                            @if(empty($data->pin))
                        ...
                        <div class="grid grid-cols-1 gap-y-4 sm:grid-cols-2 sm:gap-x-4">
                            <div class="sm:col-span-2">
                                <label for="current_pin_hash" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Hash PIN Saat Ini</label>
                                <div class="relative">
                            ...
                            <div class="col-span-1 sm:col-span-2 lg:col-span-1">
                                <label for="pin" class="block text-sm font-medium text-gray-700">Set PIN Baru / Paksa Reset PIN</label>
                                <div class="relative mt-1 rounded-md shadow-sm">
                                    ...
                                    <input type="password" name="pin" id="pin" autocomplete="off" maxlength="6" pattern="\d{6}" placeholder="Harus 6 Digit Angka"
                                           class="block w-full pl-8 rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm transition duration-150 ease-in-out font-mono tracking-widest text-lg">
                                </div>
        ...
        <div class="px-4 py-4 sm:px-6 bg-gray-50 border-t border-gray-200 flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3">
            ...
            <button type="submit" class="w-full sm:w-auto inline-flex justify-center items-center px-6 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-150 ease-in-out">
