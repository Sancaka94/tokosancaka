                    ...
                    <tr class="group hover:bg-gray-50">
                        ...
                        <td class="hidden md:table-cell px-4 py-4 align-middle whitespace-nowrap text-sm font-medium sticky-col bg-gray-50 md:bg-white border-t md:border-none toggle-target-{{$index}}">
                            ...
                            <div class="flex items-center justify-center md:justify-center space-x-3 md:space-x-3 w-full py-2 md:py-0">
                                ...
                                {{-- Cancel Order (Hanya muncul jika status Menunggu Pickup & Resi Valid) --}}
                                @if(in_array($order->status_pesanan, ['Menunggu Pickup', 'Pesanan Dibuat']) && !empty($order->resi) && !Str::startsWith($order->resi, 'REF-'))
                                    <button type="button" onclick="openModal('cancelModal{{ $order->resi }}')" class="text-gray-500 hover:text-yellow-500 transform hover:scale-110 transition" title="Batalkan via API">
                        ...
                        <div id="cancelModal{{ $order->resi }}" class="fixed inset-0 bg-gray-800 bg-opacity-60 z-[9999] hidden flex items-center justify-center">
                            <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 overflow-hidden">
                                <div class="p-4 border-b flex justify-between items-center bg-red-50">
                                    ...
                                    <button type="button" onclick="closeModal('cancelModal{{ $order->resi }}')" class="text-gray-500 hover:text-gray-800">&times;</button>
                                </div>
                                <form action="{{ route('admin.pesanan.cancel', $order->resi) }}" method="POST">
                                    @csrf
                                    <div class="p-6">
                                        <p class="text-sm text-gray-600 mb-4 whitespace-normal">Anda akan membatalkan resi <strong>{{ $order->resi }}</strong> di sistem KiriminAja. Masukkan alasan pembatalan (minimal 5 karakter).</p>
                                    ...
                                    <div class="p-4 border-t flex justify-end gap-2 bg-gray-50">
                                        <button type="button" onclick="closeModal('cancelModal{{ $order->resi }}')" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 text-sm font-medium">Tutup</button>
