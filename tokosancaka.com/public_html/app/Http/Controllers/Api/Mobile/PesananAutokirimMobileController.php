...
class PesananAutokirimMobileController extends Controller
{
    protected $baseUrl;
    protected $token;
    ...
    public function indexRiwayatMobile(Request $request)
    {
        ...
        $pesanan->getCollection()->transform(function($item) {
            $item->created_at_format = $item->created_at->format('d M Y H:i');
            return $item;
        });
    ...
    public function storeOrderMobile(Request $request)
    {
        ...
        if (!$lock->get()) {
            return response()->json([
                'success' => false,
                'message' => 'Pesanan Anda sedang diproses. Mohon jangan klik tombol submit berkali-kali.'
            ]);
        }
        ...
        try {
            ...
            $isInsurance = (bool) $request->asuransi;
            ...
            try {
                ...
                if (in_array($paymentMethod, ['potong_saldo', 'dana_binding', 'cod_barang', 'cod_ongkir'])) {
                   ...
                   // --- PERBAIKAN LOGIKA SIMPAN KE TABEL KONTAK ---
                    ...
                    return response()->json([
                        'success' => true,
                        'message' => "Pesanan Berhasil! Nomor Resi: {$awbResult['awb']} (Metode: {$metodeTampil})",
                        'data' => [
                            'awb' => $awbResult['awb'],
                            'payment_url' => null
                        ]
    ...
    private function _processDanaBindingCharge($pesanan, $total)
    {
        ...
        try {
            ...
            $headers = [
                'Authorization'          => 'Bearer ' . $accessToken,
                'Authorization-Customer' => 'Bearer ' . $tokenCustomer,
                'X-PARTNER-ID'           => $partnerIdConf,
                'X-EXTERNAL-ID'          => (string) time() . \Illuminate\Support\Str::random(6),
                'X-TIMESTAMP'            => $timestamp,
                'X-SIGNATURE'            => $signature,
