...
class ShipTicketingController extends BaseController
{
   ...
   public function shipBooking(Request $request)
    ...
    public function shipBookingDetail(Request $request)
    {
       ...
       try {
            // FILTER PENTING: Cegah numCode / PROSES masuk sebagai bookingNumber
            $bNumber = $request->bookingNumber;
            if ($bNumber === $request->numCode || $bNumber === 'PROSES') {
                $bNumber = "";
            }

            $payload = [
                'numCode'       => $request->numCode,
                'bookingDate'   => date('c', strtotime($request->bookingDate)), // Format to ISO 8601
                'bookingNumber' => $bNumber, // <-- Gunakan variabel hasil filter
                'userID'        => $this->darmawisataUserId,
                'accessToken'   => $request->accessToken
            ];
            ...
            if (isset($json['status']) && $json['status'] === 'SUCCESS') {
                ...
                // Gunakan bookingNumber dari Darmawisata (kalau null, simpan string kosong)
                $bookingNumber = $json['bookingNumber'] ?? $json['bokingNumber'] ?? $bNumber;

                // A. Jika API Darmawisata SUKSES, simpan/update payload utuh ke Database
                DB::table('ship_booking_details')->updateOrInsert(
                    ['num_code' => $request->numCode], // <-- Cari pakai numCode agar konsisten
                    [
                        'booking_number' => $bookingNumber,
    ...
    /**
     * =========================================================
     * MENERIMA WEBHOOK DARI DOKU (TIKET KAPAL LAUT PELNI)
     * =========================================================
     */
    public function handleDokuCallback($data)
    {
        try {
            $invoiceNumber = $data['order']['invoice_number']; // Contoh: SHP-8-PLN34
            ...
            // 1. Cari data pesanan di tabel Kapal
            $order = DB::table('ship_orders')->where('id', $transactionId)->first();

            if (!$order) {
                Log::warning("DOKU Webhook Kapal: Transaksi $transactionId tidak ditemukan.");
            ...
            if ($order->status !== 'HOLD') {
                Log::info("DOKU Webhook Kapal: Transaksi $transactionId sudah diproses (Status: {$order->status}).");
                return response()->json(['status' => 'success', 'message' => 'Already processed']);
            }

            // 3. Update status menjadi PAID_PROCESSING (Lunas, siap di-Issued)
            DB::table('ship_orders')->where('id', $transactionId)->update([
                'status' => 'PAID_PROCESSING', 
                'updated_at' => now()
            ]);

            Log::info("DOKU Webhook Kapal: Uang pesanan $transactionId sudah masuk Sancaka. Menunggu Issued.");
            
            // TODO: Jika ingin Auto-Issued, panggil fungsi API Ship/Issued di sini.

            return response()->json(['status' => 'success', 'message' => 'Webhook Kapal berhasil']);
            
        } catch (\Exception $e) {
            Log::error("Webhook Kapal Error: " . $e->getMessage());
