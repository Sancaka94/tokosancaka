   ...
   public function handleCallback(Request $request)
{
    ...
    public function transferToBank(Request $request)
    {
        ...
        $accessTokenB2B = $this->danaSignature->getAccessToken();
            ...
            // =====================================================================
            // [TRANSFER BANK] SCENARIO 1: UNAUTHORIZED INVALID SIGNATURE (Pemicu: Rp 22.222)
            // =====================================================================
            if ($request->amount == 22222) {
                $headersTest = $headers;
                $headersTest['X-SIGNATURE']   = "SIGNATURE_SALAH_TOTAL_43"; // Rusak total signature
                ...
                // Refund saldo lokal agar data tidak berkurang
                DB::table('Pengguna')->where('id_pengguna', $aff->id_pengguna)->increment('saldo', $request->amount);

                return back()->with('error', "TES INVALID SIGNATURE SELESAI!\nResponse: [" . ($resTest->json()['responseCode'] ?? 'Error') . "] " . ($resTest->json()['responseMessage'] ?? ''));
            }

            // =====================================================================
            // [TRANSFER BANK] SCENARIO 2: GENERAL ERROR (Pemicu: Rp 77.777)
            // =====================================================================
            if ($request->amount == 77777) {
                $bodyTest = $body;
                $bodyTest['partnerReferenceNo'] = "TRF_GEN_" . time() . rand(1000, 9999);

                // Mengirimkan data invalid di field nominal/currency demi memicu General Error dari gateway DANA
                $bodyTest['amount']['currency'] = "INVALID_XYZ";
            ...
            // =====================================================================
            // [TRANSFER BANK] SCENARIO 3: SUSPECTED FRAUD (Pemicu: Rp 99.999)
            // =====================================================================
            if ($request->amount == 99999) {
                $bodyTest = $body;
                $bodyTest['partnerReferenceNo'] = "TRF_FRAUD_" . time() . rand(1000, 9999);

                // Sandbox DANA biasanya memicu fraud jika mendeteksi instruksi nominal bernilai minus atau tidak wajar
                $bodyTest['amount']['value'] = "-1000.00";
                ...
                return back()->with('error', "TES SUSPECTED FRAUD SELESAI!\nResponse: [" . ($resTest->json()['responseCode'] ?? 'Error') . "] " . ($resTest->json()['responseMessage'] ?? ''));
            }

            // =====================================================================
            // [TRANSFER BANK] SCENARIO 4: INTERNAL SERVER ERROR (Pemicu: Rp 55.555)
            // =====================================================================
            if ($request->amount == 55555) {
                $bodyTest = $body;
                $bodyTest['partnerReferenceNo'] = "TRF_ISE_" . time() . rand(1000, 9999);

                // Memaksa internal server error DANA Sandbox dengan mengirimkan field duplikat/tipe data merusak sistem internal mereka
                $bodyTest['amount'] = "50000.00";
                ...
                return back()->with('error', "TES INTERNAL SERVER ERROR SELESAI!\nResponse: [" . ($resTest->json()['responseCode'] ?? 'Error') . "] " . ($resTest->json()['responseMessage'] ?? ''));
            }
