...
class TopUpController extends Controller
{
    ...
    public function create()
    {
        ...
        // Normalisasi struktur Tripay tanpa membuang array aslinya
        $tripayChannels = collect(is_string($tripayRaw) ? json_decode($tripayRaw, true) : $tripayRaw)
            ->map(function ($item) {
                // Ambil total fee untuk berjaga-jaga
                $flatFee = $item['total_fee']['flat'] ?? 0;

                // Gunakan array_merge agar SEMUA data asli Tripay tetap ikut ke Blade
    ...
    private function getTripayChannels()
    {
        ...
        // 🔥 GANTI NAMA CACHE MENJADI v3 AGAR CACHE LAMA TERHAPUS PAKSA
        return Cache::remember('tripay_channels_v3_' . $mode, 60 * 24, function () use ($mode) {
...
public function createPaymentDanaBinding(Transaction $transaction, $userAccount)
        ...
        try {
            ...
            if (isset($result['responseCode']) && $result['responseCode'] === '2005400') {
                ...
                if (!empty($redirectUrl)) {
                    ...
                    Log::info('LOG LOG: [DANA BINDING] Request Apply OTT ke DANA...');
                    ...
                    if (isset($resultOtt['responseCode']) && $resultOtt['responseCode'] === '2004900') {
                        ...
                        // Jika gagal, biarkan saja. User tetap bisa bayar tapi mungkin harus pencet login/masukin PIN lagi
    ...
    public function createPaymentDanaBindingWidget(Transaction $transaction, $userAccount)
    {
        ...
        $body = [
            "partnerReferenceNo" => (string) $trxId,
            "merchantId"         => config('services.dana.merchant_id'),
            "validUpTo"          => $validUpTo, // Menggunakan variabel yang sudah di-define
            "amount" => [
                "value"    => $amountValue,
                "currency" => "IDR"
            ],
            "urlParams" => [
                ...
                [
                    ...
                    "url"        => route('dana.return', ['trx_id' => $trxId]), // Ganti dengan custom app scheme (cth: myapp://dana/return) jika aplikasimu menggunakan scheme spesifik
                    "isDeeplink" => "Y"
                ]
            ],
            "payOptionDetails" => [
  ...
  /**
    ...
    /**
     * =========================================================================
     * HELPER RESPONSE (AGAR BISA BACA AJAX & REGULAR REQUEST)
     * =========================================================================
     */
