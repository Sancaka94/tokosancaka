...
<div x-data dusk="new_group_modal">
    ...
    <div
        class="relative w-full h-[410px] border  items-center justify-center border-[var(--wc-light-border)] overflow-auto bg-[var(--wc-light-primary)] sm:max-w-lg sm:rounded-lg">
            ...
            <form wire:submit="validateDetails()" class="flex flex-col  h-full p-4">
                    ...
                    <div class="flex gap-10 w-full">
                        ...
                        <div class=" my-auto">
                            ...
                            <input id='name' type="text" wire:model='name' autofocus placeholder="{{__('wirechat::new.group.inputs.name.placeholder') }}"
                                class=" w-full border-0 px-0  bg-inherit outline-hidden w-full focus:outline-hidden  focus:ring-0 hover:ring-0">
                ...
                <main class="my-5">
                    <div class=" my-auto flex flex-col gap-y-2">
                        ...
                        <textarea id='description' type="text" wire:model='description' placeholder="{{__('wirechat::new.group.inputs.description.placeholder')}}" rows="4"
                            class=" w-full resize-none rounded-lg border-[var(--wc-light-border)]    bg-inherit outline-hidden w-full focus:outline-hidden  focus:ring-0 hover:ring-0">
                        </textarea>
                ...
                <footer class="flex gap-4 justify-end mt-auto">
                    <x-wirechat::actions.close-modal>
                        <button type="button" dusk="cancel_create_new_group_button"class="font-bold cursor-pointer hover:bg-[var(--wc-light-secondary)] p-3 px-4 rounded-xl ">
                            @lang('wirechat::new.group.actions.cancel.label')
                        </button>
                    </x-wirechat::actions.close-modal>

                    <button type="submit" :disabled="!($wire.name?.trim()?.length)" dusk="next_button"
                        :class="{ 'cursor-not-allowed hover:bg-none opacity-70': !($wire.name?.trim()?.length) }"
                        class="font-bold cursor-pointer transition hover:bg-[var(--wc-light-secondary)] p-3 px-4 rounded-xl ">
            ...
            <header class=" sticky top-0 bg-[var(--wc-light-primary)] z-10 py-2">
                <div class="flex items-center pb-2">
                    ...
                    <button @click="$wire.showAddMembers=false"
                        class="p-2 ml-0 text-gray-600 rounded-full hover:text-gray-800 hover:bg-[var(--wc-light-secondary)]">
                    ...
                    <button 
                        ...
                        wire:target='create'
                        class="p-2 disabled:cursor-not-allowed  ml-0 text-gray-600 cursor-pointer rounded-full hover:text-gray-800 hover:bg-[var(--wc-light-secondary)]">
                ...
                {{-- Search input --}}
                <section class="flex flex-wrap items-center px-0 border-b border-[var(--wc-light-secondary)]">
                    <input type="search" id="users-search-field" wire:model.live.debounce='search' autocomplete="off"
                        ...
                        class=" w-full border-0 w-auto outline-hidden focus:outline-hidden bg-[var(--wc-light-primary)] bg-none rounded-lg focus:ring-0 hover:ring-0">
                </section>
                     ...
                     class="flex w-full overflow-x-auto gap-3">
                        ...
                        @if ($selectedMembers)
                            ...
                            @foreach ($selectedMembers as $key => $member)
                                <li class="flex items-center text-nowrap min-w-fit px-2 py-1 text-sm font-medium text-gray-800  bg-[var(--wc-light-secondary)] rounded-sm "
                                    wire:key="selected-member-{{ $member->id }}">
                                    {{ $member->display_name }}
                                    <button type="button"
                                        wire:click="toggleMember('{{ $member->id }}',{{ json_encode(get_class($member)) }})"
                                        class="flex items-center p-1 ms-2 text-sm text-gray-400 bg-transparent rounded-xs hover:bg-[var(--wc-light-secondary)]  hover:text-gray-900 "
                                        aria-label="Remove">
