...
// -------------------------------------------------------------------------
    ...
    public function handleDanaNotify(Request $request)
{
    // 1. LOG INCOMING REQUEST
    Log::info("[DANA-WEBHOOK] Hit Masuk", $request->all());

    try {
        $data = $request->all();

        // Ambil parameter penting
        $status = $data['orderStatus'] ?? ''; // FINISHED / SUCCESS
        $refNo  = $data['partnerReferenceNo'] ?? '';

        // Ambil nominal dari payload DANA (pastikan formatnya angka)
        // Struktur: amount: { value: "1000.00", currency: "IDR" }
        $amountData = $data['amount']['value'] ?? 0;
        $paidAmount = (float) $amountData;

        // 2. CARI TRANSAKSI PENDING
        // Kita kunci status 'PENDING' agar tidak diproses double
        ...
        if (!$trx) {
            Log::warning("[DANA-WEBHOOK] Transaksi skip (Tidak ditemukan/Sudah Sukses): $refNo");
            // Tetap return sukses ke DANA supaya tidak dikirim ulang terus menerus
            return response()->json(['responseCode' => '2000000', 'responseMessage' => 'Success']);
        }

        // 3. JIKA STATUS SUKSES DARI DANA
        if ($status === 'FINISHED' || $status === 'SUCCESS') {
            ...
            try {
                ...
                // B. UPDATE SALDO AFFILIATE (Core Logic)
                // Kita gunakan method increment/decrement atomic agar aman dari race condition

                // 1. Tambah Balance Internal (Topup Berhasil)
                DB::table('affiliates')
                    ->where('id', $trx->affiliate_id)
                    ->increment('balance', $paidAmount);

                // 2. Kurangi Saldo DANA User (Karena dipakai bayar)
                // Opsional: Sebaiknya dicek dulu agar tidak minus, tapi logic decrement di SQL aman
                DB::table('affiliates')
                    ->where('id', $trx->affiliate_id)
                    ->decrement('dana_user_balance', $paidAmount);

                DB::commit();

                Log::info("[DANA-WEBHOOK] SUKSES! Saldo ID {$trx->affiliate_id} bertambah: Rp {$paidAmount}");

            } catch (\Exception $e) {
                DB::rollBack();
                Log::error("[DANA-WEBHOOK] Gagal Update DB: " . $e->getMessage());
                return response()->json(['responseCode' => '500', 'message' => 'Internal Server Error'], 500);
            }

        } elseif ($status === 'FAILED' || $status === 'EXPIRED') {
            // Jika Gagal, update status saja, saldo jangan diubah
            DB::table('dana_transactions')
                ->where('id', $trx->id)
                ->update([
                    'status' => 'FAILED',
            ...
            Log::info("[DANA-WEBHOOK] Transaksi Gagal/Expired: $refNo");
        }

        // 4. RESPONSE WAJIB KE DANA (Format JSON Baku)
        return response()->json([
        ...
        Log::error("[DANA-WEBHOOK] Exception Fatal: " . $e->getMessage());
        return response()->json(['responseCode' => '500', 'message' => 'Error'], 500);
    }
}
    // -------------------------------------------------------------------------
    //  TAMBAHKAN KODE INI (END)
    // -------------------------------------------------------------------------
}
