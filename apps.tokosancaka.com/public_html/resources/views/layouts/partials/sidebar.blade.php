{{--
    ...
    Deskripsi: Sidebar Livewire Compatible
--}}
...
<aside
    ...
    {{-- [LOGIC PENYELAMAT] AMBIL SUBDOMAIN MANUAL --}}
    @php
    ...
    <div class="flex items-center h-20 px-6 border-b border-slate-50 flex-shrink-0">
        <a href="{{ route('dashboard', $params) }}" class="flex items-center gap-3 group" wire:navigate>
    ...
    <div class="px-6 pb-4 pt-2 text-center">
        <a href="{{ route('admin.changelog') }}" class="group block hover:bg-slate-50 rounded-lg py-2 transition-colors cursor-pointer" wire:navigate>
    ...
    <nav class="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto custom-scrollbar">
        ...
        {{-- DASHBOARD --}}
        <a href="{{ route('dashboard', $params) }}" wire:navigate
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200
           {{ request()->routeIs('dashboard') ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'text-slate-600 hover:bg-slate-50 hover:text-blue-600' }}">
            <i class="fas fa-th-large w-5 text-center {{ request()->routeIs('dashboard') ? 'text-white' : 'text-slate-400' }}"></i>
            <span>Dashboard</span>
        </a>

        {{-- STATUS LANGGANAN (SUPER ADMIN) --}}
        @if(in_array(Auth::user()->role, ['super_admin']))
            @if(Route::has('tenant.dashboard'))
            <a href="{{ route('tenant.dashboard', $params) }}" wire:navigate
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 mt-1
               {{ request()->routeIs('tenant.dashboard') ? 'bg-emerald-50 text-emerald-600 font-bold border border-emerald-100' : 'text-slate-600 hover:bg-emerald-50 hover:text-emerald-600' }}">
        ...
        @if(in_array(Auth::user()->role, ['super_admin', 'admin', 'staff', 'operator']))
            ...
            <a href="{{ route('orders.create', $params) }}" wire:navigate
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200
               {{ request()->routeIs('orders.create') ? 'bg-red-600 text-white shadow-lg shadow-red-200' : 'text-slate-600 hover:bg-slate-50 hover:text-red-600' }}">
                <i class="fas fa-plus-circle w-5 text-center"></i>
                <span>Buat Pesanan</span>
            </a>

            <a href="{{ route('orders.index', $params) }}" wire:navigate
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200
               {{ request()->routeIs('orders.index') || request()->routeIs('orders.show') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50 hover:text-blue-600' }}">
        ...
        @if(in_array(Auth::user()->role, ['super_admin', 'admin', 'keuangan', 'finance']))
            <a href="{{ route('reports.index', $params) }}" wire:navigate
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200
               {{ request()->routeIs('reports.index') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50 hover:text-blue-600' }}">
        ...
        {{-- DANA DASHBOARD --}}
        @if(in_array(Auth::user()->role, ['super_admin', 'admin', 'keuangan', 'finance']))
            <a href="{{ route('dana.dashboard', $params) }}" wire:navigate
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200
               {{ request()->routeIs('dana.dashboard') ? 'bg-sky-50 text-sky-600' : 'text-slate-600 hover:bg-slate-50 hover:text-sky-600' }}">
        ...
        @if(in_array(Auth::user()->role, ['super_admin', 'admin']))
            <a href="{{ route('affiliate.index', $params) }}" wire:navigate
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200
               {{ request()->routeIs('affiliate.*') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50 hover:text-blue-600' }}">
        ...
        {{-- DATA TENANT --}}
        @if(in_array(Auth::user()->role, ['super_admin']))
            <a href="{{ url('admin/list-customer') }}" wire:navigate
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200
               {{ Request::is('admin/list-customer*') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50 hover:text-blue-600' }}">
        ...
        {{-- PRODUK --}}
        @if(in_array(Auth::user()->role, ['super_admin', 'admin', 'staff', 'operator']))
            <a href="{{ route('products.index', $params) }}" wire:navigate
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200
               {{ request()->routeIs('products.*') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50 hover:text-blue-600' }}">
        ...
        @if(in_array(Auth::user()->role, ['super_admin', 'admin', 'keuangan', 'finance']))
            ...
            <div x-data="{ open: {{ request()->routeIs('finance.*') ? 'true' : 'false' }} }">
                ...
                <div x-show="open" x-collapse x-cloak class="mt-1 ml-7 border-l-2 border-slate-100 space-y-1">
                    <a href="{{ route('finance.index', $params) }}" wire:navigate
                    ...
                    <a href="{{ route('finance.laba_rugi', $params) }}" wire:navigate
                       class="block pl-6 pr-3 py-2 text-xs font-medium rounded-r-xl transition-all
                    ...
                    <a href="{{ route('finance.tahunan', $params) }}" wire:navigate
                       class="block pl-6 pr-3 py-2 text-xs font-medium rounded-r-xl transition-all
                    ...
                    <a href="{{ route('finance.sync', $params) }}" wire:navigate
        ...
        @if(in_array(Auth::user()->role, ['super_admin', 'admin']))
            <div x-data="{ open: {{ request()->routeIs('employees.*') ? 'true' : 'false' }} }">
                ...
                <div x-show="open" x-collapse x-cloak class="mt-1 ml-7 border-l-2 border-slate-100 space-y-1">
                    <a href="{{ route('employees.index', $params) }}" wire:navigate
                       class="block pl-6 pr-3 py-2 text-xs font-medium rounded-r-xl transition-all
                    ...
                    <a href="{{ route('employees.create', $params) }}" wire:navigate
                       class="block pl-6 pr-3 py-2 text-xs font-medium rounded-r-xl transition-all
        ...
        <div x-data="{ open: {{ request()->routeIs('profile.*') ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-collapse x-cloak class="mt-1 ml-7 border-l-2 border-slate-100 space-y-1">
                <a href="{{ route('profile.index', $params) }}" wire:navigate
                   class="block pl-6 pr-3 py-2 text-xs font-medium rounded-r-xl transition-all
                ...
                <a href="{{ route('profile.edit', $params) }}" wire:navigate
                   class="block pl-6 pr-3 py-2 text-xs font-medium rounded-r-xl transition-all
    ...
    <div class="p-4 border-t border-slate-50 flex-shrink-0">
        <div class="bg-slate-50 rounded-2xl p-3 mb-3 flex items-center gap-3">
            {{-- Foto Profile --}}
            <div class="w-10 h-10 rounded-xl flex-shrink-0 overflow-hidden flex items-center justify-center border border-blue-50 bg-white">
                @if(Auth::user()->logo)
                    <img src="{{ asset('storage/' . Auth::user()->logo) }}" class="w-full h-full object-cover" alt="User Logo">
                @else
