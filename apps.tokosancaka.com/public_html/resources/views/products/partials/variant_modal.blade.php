...
<div x-show="variantModalOpen"
    ...
    <div class="bg-white w-full max-w-4xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[85vh]"
            ...
            <div x-show="!isLoadingVariant">
                ...
                <div class="flex justify-between items-end mb-4">
                    <div>
                        ...
                        <p class="text-[10px] text-slate-400">Atur harga, diskon, stok, dan barcode berbeda untuk setiap varian.</p>
                    </div>
                ...
                <div x-show="variants.length > 0" class="space-y-2">
                    <div class="grid grid-cols-12 gap-2 text-[9px] font-bold text-slate-400 uppercase tracking-wider px-1">
                        <div class="col-span-2">Nama Varian <span class="text-red-500">*</span></div>
                        <div class="col-span-2">Barcode</div>
                        <div class="col-span-2">Harga (Rp) <span class="text-red-500">*</span></div>
                        <div class="col-span-3 text-center">Diskon (Jenis & Nilai)</div>
                            ...
                            <div class="grid grid-cols-12 gap-2 items-center p-3 bg-slate-50 border-b border-slate-200">
                                ...
                                <div class="col-span-2">
                                    <input type="text" x-model="variant.name" placeholder="Cth: Merah"
                                           class="w-full px-2 py-2 rounded-lg border-slate-300 text-xs focus:ring-2 focus:ring-indigo-500 font-bold text-slate-700 bg-white transition-all">
                                </div>
                                ...
                                <div class="col-span-2">
                                    <input type="number" x-model="variant.price" placeholder="0"
                                           class="w-full px-2 py-2 rounded-lg border-emerald-200 text-xs focus:ring-2 focus:ring-emerald-500 text-emerald-700 font-bold bg-white text-right transition-all">
                                </div>

                                {{-- Diskon (Jenis & Value) --}}
                                <div class="col-span-3 flex gap-1">
                                    <select x-model="variant.discount_type" class="w-1/2 px-1 py-2 rounded-lg border-slate-300 text-xs focus:ring-2 focus:ring-indigo-500 bg-white font-bold text-slate-600 appearance-none text-center">
                                        <option value="percent">%</option>
                                        <option value="nominal">Rp</option>
                                    </select>
                                    <input type="number" x-model="variant.discount_value" placeholder="0" min="0" step="any"
                                           class="w-1/2 px-2 py-2 rounded-lg border-slate-300 text-xs focus:ring-2 focus:ring-indigo-500 bg-white text-center transition-all">
                                </div>
                                ...
                                <div class="col-span-2">
                                    <input type="number" x-model="variant.stock" placeholder="0" :readonly="variant.sub_variants && variant.sub_variants.length > 0"
                                           ...
                                           class="w-full px-2 py-2 rounded-lg border-slate-300 text-xs focus:ring-2 focus:ring-indigo-500 text-center transition-all">
                                </div>
                                ...
                                <div class="space-y-2 pl-4 border-l-2 border-orange-100">
                                    <template x-for="(sub, subIndex) in variant.sub_variants" :key="subIndex">
                                        <div class="grid grid-cols-12 gap-2 items-center">
                                            ...
                                            {{-- Nama & Barcode --}}
                                            <div class="col-span-2">
                                                <input type="text" x-model="sub.name" placeholder="Nama" class="w-full px-2 py-1.5 text-xs border border-slate-300 rounded focus:ring-1 focus:ring-orange-500">
                                            </div>
                                            <div class="col-span-2">
                                            ...
                                            {{-- Diskon Sub Varian --}}
                                            <div class="col-span-3 flex gap-1">
                                                <select x-model="sub.discount_type" class="w-1/2 px-1 py-1.5 rounded border border-slate-300 text-xs focus:ring-1 focus:ring-orange-500 bg-white font-bold text-slate-600 appearance-none text-center">
                                                    <option value="percent">%</option>
                                                    <option value="nominal">Rp</option>
                                                </select>
                                                <input type="number" x-model="sub.discount_value" placeholder="Diskon" min="0" step="any"
                                                       class="w-1/2 px-2 py-1.5 border border-slate-300 rounded text-xs focus:ring-1 focus:ring-orange-500 bg-white text-center transition-all">
                                            </div>
                                            ...
                                            <div class="col-span-1">
                                                <input type="number" x-model="sub.weight" placeholder="Gram" class="w-full px-2 py-1.5 text-xs border border-slate-300 rounded focus:ring-1 focus:ring-orange-500 text-center" title="Berat dalam Gram">
                                            </div>
                                    ...
                                    <template x-if="!variant.sub_variants || variant.sub_variants.length === 0">
                                        <p class="text-[9px] text-slate-400 italic">Tidak ada sub varian. Harga, diskon & stok mengikuti varian utama di atas.</p>
                                    </template>
                                </div>
                            </div>
