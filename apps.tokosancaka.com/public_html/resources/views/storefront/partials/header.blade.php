    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-20">
            ...
            <a href="{{ route('storefront.index', $subdomain) }}" class="flex items-center gap-3">
                ...
                @else
                    <div class="w-10 h-10 bg-red-600 rounded-[12px] flex-shrink-0 flex justify-center items-center text-white font-black text-xl shadow-inner">
            ...
            <div class="hidden md:flex flex-1 max-w-lg mx-8">
                <form action="{{ route('storefront.index', $subdomain) }}" method="GET" class="w-full relative">
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari produk di toko ini..."
                           class="w-full bg-gray-100 border-none rounded-full py-2.5 pl-5 pr-12 focus:ring-2 focus:ring-red-500 outline-none text-sm transition">
            ...
            <div class="flex items-center gap-2 md:gap-4">
                ...
                <div x-data="{ openCategory: false }" class="relative">
                    ...
                    <div x-show="openCategory"
                        ...
                        <div class="flex justify-between items-center mb-3 pb-2 border-b border-gray-50">
                            ...
                            <a href="{{ route('storefront.index', $subdomain) }}" class="text-[10px] text-red-600 font-bold hover:underline">Lihat Semua</a>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-2 max-h-[60vh] overflow-y-auto pr-1 mt-2">
                            ...
                            @forelse($navCategories as $cat)
                                <a href="{{ route('storefront.index', ['subdomain' => $subdomain, 'category' => $cat->id]) }}"
                                   class="flex items-center justify-between p-2 hover:bg-red-50 rounded-lg transition group">
                                    <div class="flex items-center gap-2 overflow-hidden">
                                        <div class="w-8 h-8 rounded-md bg-gray-100 flex items-center justify-center text-gray-500 group-hover:bg-red-200 group-hover:text-red-700 transition shrink-0">
                                            <i data-lucide="folder" class="w-4 h-4"></i>
                                        </div>
                                        <span class="text-xs font-medium text-gray-700 group-hover:text-red-800 truncate">
                                            {{ $cat->name }}
                                        </span>
                                    </div>
                                    {{-- JUMLAH ITEM PRODUK --}}
                                    <span class="text-[10px] text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full group-hover:bg-red-100 group-hover:text-red-700 transition shrink-0 font-bold">
                ...
                {{-- Cart Icon --}}
                <a href="{{ route('storefront.cart', $subdomain) }}" class="relative p-2 text-gray-600 hover:text-red-600 transition group">
                ...
                @auth
                    <a href="{{ url('/dashboard') }}" class="flex items-center gap-1.5 bg-red-600 text-white px-3 py-1.5 sm:px-4 sm:py-2 rounded-[12px] text-xs sm:text-sm font-bold hover:bg-red-700 shadow-sm transition active:scale-95">
