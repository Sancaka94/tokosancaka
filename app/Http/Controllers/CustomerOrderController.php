...
class CustomerOrderController extends Controller
{
     ...
     * Logika ini diambil langsung dari referensi CekOngkirController Anda.
     */
    public function searchAddressApi(Request $request, KiriminAjaService $kirimaja)
    {
        ...
        try {
            ...
            // Mengembalikan data mentah dari API sesuai referensi
    ...
    public function store(Request $request, KiriminAjaService $kirimaja)
    {
        ...
        try {
            ...
            if (in_array($validatedData['payment_method'], ['COD', 'CODBARANG'])) {
                ...
                $orderItemsPayload = $this->_prepareOrderItemsPayload($shipping_cost, $ansuransi_fee, $validatedData['item_price']);
                $response = $this->_createTripayTransaction($request, $pesanan, ($total_paid - $validatedData['item_price']), $orderItemsPayload);
...
\App\Services\FonnteService::sendMessage($senderWa, $senderMessage);
    ...
    {
        $contactData = function ($prefix) use ($request) {
            return [ 'nama' => $request->input("{$prefix}_name"), 'alamat' => $request->input("{$prefix}_address"), 'province' => $request->input("{$prefix}_province"), 'regency' => $request->input("{$prefix}_regency"), 'district' => $request->input("{$prefix}_district"), 'village' => $request->input("{$prefix}_village"), 'postal_code' => $request->input("{$prefix}_postal_code"), ];
        };
        if ($request->has('save_sender')) Kontak::updateOrCreate(['no_hp' => $request->sender_phone], array_merge($contactData('sender'), ['tipe' => 'Pengirim']));
        if ($request->has('save_receiver')) Kontak::updateOrCreate(['no_hp' => $request->receiver_phone], array_merge($contactData('receiver'), ['tipe' => 'Penerima']));
    }
    ...
    private function _prepareOrderItemsPayload(int $shipping_cost, int $ansuransi_fee, int $item_price): array
    {
        $payload = [];
        $payload[] = ['sku' => 'SHIPPING', 'name' => 'Ongkos Kirim', 'price' => $shipping_cost, 'quantity' => 1];
        if ($ansuransi_fee > 0) $payload[] = ['sku' => 'INSURANCE', 'name' => 'Biaya Asuransi', 'price' => $ansuransi_fee, 'quantity' => 1];
        return $payload;
    }
    ...
    private function _createKiriminAjaOrder(Request $request, Pesanan $pesanan, KiriminAjaService $kirimaja, array $senderData, array $receiverData, int $cod_value): array
    {
        ...
        if (in_array($serviceGroup, ['instant', 'sameday'])) { /* ... logika instant ... */ } 
        else {
            ...
            $payload = [
                ...
                'packages' => [[
                    ...
                    'weight' => $request->weight, 'width' => $request->width, 'height' => $request->height, 'length' => $request->length,
