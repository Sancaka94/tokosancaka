...
class PesananController extends Controller
{
    ...
    public function store(Request $request, KiriminAjaService $kirimaja)
    {
        try {
            ...
            $this->_sendWhatsappNotification($pesanan, $validatedData, $shipping_cost, $pesanan->insurance_cost, $pesanan->cod_fee, $notification_total);
    ...
    public static function processPesananCallback($merchantRef, $status, $callbackData)
    {
        ...
        // Lock record Pesanan untuk mencegah race condition
        $pesanan = Pesanan::where('nomor_invoice', $merchantRef)->lockForUpdate()->first();

        if (!$pesanan) {
            Log::error('Tripay Callback (Pesanan SCK-): Not found.', ['merchant_ref' => $merchantRef]);
            return; // Selesai, tidak ada error
        }
        ...
        if ($status === 'PAID') {
            ...
            try {
                // Buat instance controller untuk akses method private (jika diperlukan)
                 // Sebaiknya pindahkan helper ke Service atau Trait jika sering dipakai lintas controller
                $instance = new self();

                // Ambil data dari pesanan (sudah ada di $pesanan)
                $validatedData = $pesanan->toArray(); // Ambil semua data dari model

                // Siapkan data alamat (ambil langsung dari model $pesanan)
                $senderAddressData = [
                ...
                if (($kiriminResponse['status'] ?? false) !== true) {
                    ...
                    $pesanan->status = 'Pembayaran Lunas (Gagal Auto-Resi)'; // Update status agar jelas
                    $pesanan->status_pesanan = 'Pembayaran Lunas (Gagal Auto-Resi)';
                    // TODO: Kirim notifikasi ke admin tentang kegagalan ini
                } else {
                    // KiriminAja SUKSES
                    $pesanan->resi = $kiriminResponse['result']['awb_no'] ?? ($kiriminResponse['results'][0]['awb'] ?? null);
                    $pesanan->status = 'Menunggu Pickup'; // Update status jadi Menunggu Pickup
                    $pesanan->status_pesanan = 'Menunggu Pickup';
                    Log::info('Tripay Callback (Pesanan SCK-): KiriminAja Order SUCCESS.', ['invoice' => $merchantRef, 'resi' => $pesanan->resi]);
                    // Mungkin kirim notifikasi WA update resi ke customer?
                }
                $pesanan->save(); // Simpan status akhir dan resi

            } catch (Exception $e) {
                // Tangani error saat proses KiriminAja
    ...
    private function _sanitizePhoneNumber(string $phone): string
    {
        // ... (fungsi _sanitizePhoneNumber tetap sama) ...
         ...
         if (Str::startsWith($phone, '62')) { /* ... */ return '0' . substr($phone, 2); }
    ...
    private function _sendWhatsappNotification(
        ...
        if (empty($validatedData['sender_phone']) || empty($validatedData['receiver_phone'])) {
            Log::warning('Nomor telepon kosong, notifikasi WA dibatalkan.', ['invoice' => $pesanan->nomor_invoice]);
            return;
        }

        // Ambil nomor asli sebelum sanitasi untuk ditampilkan
        $displaySenderPhone = $request->input('sender_phone') ?? $validatedData['sender_phone_original'] ?? $pesanan->sender_phone; // Ambil dari request jika ada, fallback
        $displayReceiverPhone = $request->input('receiver_phone') ?? $validatedData['receiver_phone_original'] ?? $pesanan->receiver_phone; // Ambil dari request jika ada, fallback
        ...
        // Tampilkan Resi jika sudah ada (bukan saat baru dibuat & menunggu bayar)
        if ($pesanan->resi) {
            $detailPaket .= "\nResi: *" . $pesanan->resi . "*";
        } else {
             $detailPaket .= "\nResi: -"; // Tampilkan strip jika belum ada
        }


        $rincianBiaya = "*Rincian Biaya:*\n- Ongkir: Rp " . number_format($shipping_cost, 0, ',', '.');
         // Tampilkan nilai barang dari $validatedData jika tersedia (lebih akurat saat store), fallback ke $pesanan->item_price
        ...
        if ($cod_fee > 0) $rincianBiaya .= "\n- Biaya COD: Rp " . number_format($cod_fee, 0, ',', '.');
         ...
         if ($pesanan->payment_status === 'PAID' || $pesanan->status === 'Menunggu Pickup' || $pesanan->status === 'Diproses' || $pesanan->status === 'Terkirim') {
             $statusBayar = "✅ Lunas";
         ...
         } elseif ($pesanan->status === 'Gagal Bayar' || $pesanan->status === 'Kadaluarsa') {
