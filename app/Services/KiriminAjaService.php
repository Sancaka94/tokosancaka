...
class KiriminAjaService
{
    ...
    public function trackExpress(string $orderId)
    {
        $response = Http::withToken($this->token)
            ->post("{$this->baseUrl}/api/mitra/tracking", [
                'order_id' => $orderId,
            ]);
    ...
    public function trackInstant(string $orderId)
    {
        $response = Http::withToken($this->token)
            ->get("{$this->baseUrl}/api/mitra/v4/instant/tracking/{$orderId}");

        return $response->json();
    }

    public function track(string $serviceType, string $orderId)
    {
        return $serviceType == 'instant'
            ? $this->trackInstant($orderId)
            : $this->trackExpress($orderId);
    }
