...
$all_trx =$conn->query("SELECT mahasiswa_id, tanggal_setor, nominal FROM transaksi WHERE jenis='masuk' ORDER BY tanggal_setor DESC");
$trx_data = [];
if ($all_trx) {
    while($row_trx = $all_trx->fetch_assoc()) {$trx_data[$row_trx['mahasiswa_id']][] =$row_trx;
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Laporan Keuangan KAS HIMA - IAI NGAWI</title>
    ...
    <div class="max-w-7xl mx-auto">
        <header class="mb-8 flex flex-col md:flex-row md:items-center justify-between border-b border-gray-200 pb-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold tracking-tight">Laporan Keuangan Kas HIMA Prodi HKI</h1>
                    <p class="text-sm text-gray-500">Institut Agama Islam (IAI) Ngawi</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
            ...
            <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                <div class="text-gray-500 text-sm font-medium mb-1">Kekurangan </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
            <div class="p-5 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                <h2 class="text-lg font-semibold flex items-center gap-2">
                    <i class="ph ph-users"></i> Laporan Uang Terkumpul Per Mahasiswa
                </h2>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-sm divide-y divide-gray-100">
                        ...
                        if($query_rekap) while($row =$query_rekap->fetch_assoc()):
                            $mahasiswa_id =$row['id'];
                            // Mengambil data history spesifik mahasiswa ini & mengubahnya menjadi JSON
                            $history_json = isset($trx_data[$mahasiswa_id]) ? json_encode($trx_data[$mahasiswa_id]) : '[]';
                        ?>
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 text-gray-500"><?= $no++ ?></td>
                            <td class="px-6 py-4 font-medium text-gray-900"><?= htmlspecialchars($row['nama_mahasiswa']) ?></td>
                            <td class="px-6 py-4 text-gray-500">Semester <?= htmlspecialchars($row['semester']) ?></td>
                            <td class="px-6 py-4 font-bold text-right">Rp <?= number_format($row['total_setor'], 0, ',', '.') ?></td>
                            <td class="px-6 py-4 text-center">
                                <button onclick="openDetailModal('<?= htmlspecialchars($row['nama_mahasiswa'], ENT_QUOTES) ?>', <?= $row['total_setor'] ?>, '<?= htmlspecialchars($history_json, ENT_QUOTES) ?>')" class="bg-teal-50 hover:bg-teal-100 text-teal-600 p-2 rounded-md transition-colors inline-flex items-center justify-center" title="Lihat Rincian Setoran">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-sm divide-y divide-gray-100">
                        ...
                        ");
                        if($query_mutasi &&$query_mutasi->num_rows > 0):
                            while($mutasi =$query_mutasi->fetch_assoc()):
                                $is_masuk =$mutasi['jenis'] == 'masuk';
                                
                                // Format text Keterangan
                                $teks_keterangan = "";
                                if($is_masuk) {$teks_keterangan = "Setoran: " . (!empty($mutasi['nama_mahasiswa']) ?$mutasi['nama_mahasiswa'] : "Umum");
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 font-medium text-gray-900"><?= htmlspecialchars($teks_keterangan) ?></td>
        ...
        <div class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden flex flex-col max-h-full">
            <div class="p-5 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                <h3 class="text-lg font-semibold text-gray-900">
                    Rincian Setoran: <span id="modalNamaMahasiswa" class="text-teal-600 ml-1"></span>
                </h3>
