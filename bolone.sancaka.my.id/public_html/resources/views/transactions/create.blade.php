...
<body class="bg-gray-50 text-gray-900 font-sans antialiased">
    ...
    <!-- Ditambahkan mx-4 md:mx-auto dan p-4 md:p-8 agar tidak menempel di tepi layar HP -->
    <div class="max-w-5xl mx-4 md:mx-auto p-4 md:p-8 mt-4 md:mt-6">
        
        <!-- Header -->
        <div class="mb-8 border-b border-gray-200 pb-4 flex justify-between items-center relative">
            
            <h1 class="text-xl md:text-2xl font-semibold tracking-tight text-black">Input Data Wilayah Kota</h1>
            ...
            <div id="actionMenu" class="hidden absolute top-12 right-0 z-50 w-56 p-4 flex-col gap-3 bg-white border border-gray-200 rounded-lg shadow-xl md:flex md:static md:w-auto md:p-0 md:flex-row md:items-center md:gap-4 md:bg-transparent md:border-none md:shadow-none">
                
                <!-- Tombol Kembali -->
                <a href="{{ route('cities.index') }}" class="w-full md:w-auto justify-center px-4 py-2 bg-black text-white text-sm font-medium rounded-md hover:bg-gray-800 transition-colors shadow-sm inline-flex items-center">
                    &larr; Kembali
                </a>
                ...
                <form method="POST" action="{{ route('logout') }}" class="inline-block m-0 w-full md:w-auto">
                    ...
                    <button type="submit" onclick="return confirm('Apakah Anda yakin ingin keluar?');" 
                            class="w-full justify-center px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-50 transition-colors shadow-sm inline-flex items-center gap-2">
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-4 md:p-6 mb-8">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-5 gap-4">
                ...
                <a href="{{ route('transactions.example') }}" class="w-full md:w-auto justify-center inline-flex items-center gap-2 px-4 py-2 text-xs font-medium text-black bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors shadow-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form action="{{ route('transactions.import') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-4">
                ...
                <div class="w-full">
                    <label for="file-upload" id="drop-zone" class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 hover:border-black transition-all duration-200">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 pointer-events-none text-center px-2">
                            ...
                            <p id="file-name" class="mt-3 px-3 py-1 bg-black text-white text-xs font-semibold rounded-md hidden truncate max-w-[250px] sm:max-w-md"></p>
                        </div>
                ...
                <div class="flex justify-end border-t border-gray-100 pt-4 mt-2">
                    <button type="submit" class="w-full md:w-auto px-6 py-2.5 bg-black text-white text-sm font-medium rounded-md hover:bg-gray-800 transition-colors shadow-sm focus:ring-2 focus:ring-offset-2 focus:ring-black">
                        Upload & Proses Data
                    </button>
                </div>
            </form>
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-4 md:p-6 mb-10">
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-4 md:p-5 mb-6">
            <form action="{{ url()->current() }}" method="GET" id="filterForm" class="flex flex-col md:flex-row gap-4 md:items-end">
                ...
                <div class="flex gap-2 w-full md:w-auto pt-2 md:pt-0">
                    ...
                    <a href="{{ url()->current() }}" class="flex-1 md:flex-none px-5 py-2 bg-white border border-gray-300 text-black text-sm font-medium rounded-md hover:bg-gray-50 transition-colors shadow-sm text-center">
                        Reset
                    </a>
                </div>
            </form>
        ...
        <form action="{{ route('transactions.bulk-delete') }}" method="POST" id="bulkDeleteForm">
            ...
            <!-- Tabel: Ditambahkan overflow-x-auto agar responsif di HP -->
            <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-x-auto mb-6">
                <table class="min-w-full divide-y divide-gray-200 text-sm whitespace-nowrap">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-4 md:px-6 py-3 text-left font-medium text-gray-500 uppercase tracking-wider w-16">No</th>
                            <th scope="col" class="px-4 md:px-6 py-3 text-left font-medium text-gray-500 uppercase tracking-wider">Nama Kota</th>
                            <th scope="col" class="px-4 md:px-6 py-3 text-left font-medium text-gray-500 uppercase tracking-wider">Tanggal</th>
                            <th scope="col" class="px-4 md:px-6 py-3 text-left font-medium text-gray-500 uppercase tracking-wider">Jumlah</th>
                            <th scope="col" class="px-4 md:px-6 py-3 text-right font-medium text-gray-500 uppercase tracking-wider w-32">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 md:px-6 py-4 whitespace-nowrap text-gray-500">{{ $transactions->firstItem() + $index }}</td>
                            <td class="px-4 md:px-6 py-4 whitespace-nowrap font-medium text-black">{{ $transaction->city->nama_kota ?? '-' }}</td>
                            <td class="px-4 md:px-6 py-4 whitespace-nowrap text-gray-500">{{ \Carbon\Carbon::parse($transaction->tanggal)->format('d/m/Y') }}</td>
                            <td class="px-4 md:px-6 py-4 whitespace-nowrap text-gray-900 font-semibold">{{ number_format($transaction->jumlah, 0, ',', '.') }}</td>
                            <td class="px-4 md:px-6 py-4 whitespace-nowrap text-right flex justify-end gap-4 items-center">
